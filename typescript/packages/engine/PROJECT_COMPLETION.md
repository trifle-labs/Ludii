...
## Update 206 (2026-06-11) — Faithful stacking/step machinery (experimental Seesaw partial); audit-stub SitesDistance.stepMove ported
- experimental/Seesaw (orthogonal) advanced ply 0 -> 31/50 through a stack of faithful fixes; full pass blocked on heterogeneous-stack whatStacks materialization (see below). The supporting fixes are all correct Java behavior and ship regression-free:
  - CountStack.ts REWRITE (@java CountStack.eval): per-level walk binding (to)/(level), honoring if:/stop:/FromBottom|FromTop — previously returned the flat count and ignored the condition entirely (Seesaw's "StackSize" counts only Disc levels). Count.ts wires If/stop/direction through constructStack.
  - SitesDistance.stepMove BFS PORTED (@java SitesDistance.java:140-215 + stepMove helper) — was a documented `return []` stub; now expands a custom (step ...) move's directions per frontier site, keeping goRule-passing targets, collecting depths in [min,max] (Seesaw's "EmptyInRange").
  - Do.ts: TempContext now copies the EVAL CONTEXT (from/to/level/...) into the derived context (@java TempContext copy ctor) so `next` reads ForEachPiece's (from) binding.
  - ForEachPiece roleToIntFunction "Shared" = numPlayers+1 (already shipped Update 205); SitesOccupied top:False per-level scan + ArgCompiler wiring the @Opt @Name Boolean `top` through the occupied intercept (was hardcoded null -> defaulted True).
  - ActionMove numLevel substack (@java ActionSubStackMove): move only the TOP numLevel levels, pushed onto `to` in original order; FromTo evaluates count as numLevel when stack:True. Seesaw Draughts (substack) re-verified 2/2.
  - Game.ts heterogeneous start-stack staging: (place Stack A)+(place Stack B) at one site build a real multi-level stack (replayed as withStackPush after State construction). CRITICAL: only stages a NEW level when component/owner DIFFERS — PlaceMonotonousStack calls placePieces `count` times with the SAME piece (Backgammon's 5-checker points), which must stay a flat countAt pile. (This guard was the fix after the first attempt regressed Backgammon.)
- DEFERRED (experimental Seesaw blocker): the deployed [Hex,Disc] stacks need whatStacks materialized at EVERY level even when component id == owner id. The state.ts `wantWhat = (what !== undefined)` + top-from-`whats`-channel backfill that achieves this REGRESSES Bashni-class stacking draughts (Seesaw Draughts 2/2 -> MM). Needs a non-lazy per-level whatStacks model that preserves the existing games; parked rather than ship a regression. experimental Seesaw stays MM.
- Battery 72/72 OUTCOME_OK (Backgammon, Seesaw Draughts, Bashni, Lasca, Fenix all green), units 194/0 — read before commit. war/leaping now 220/226 (Crand/Pasang/Laram Wali cleared this window; residual Spoing[3D SitesSupport], Throngs[deprioritized], experimental/Seesaw[deferred above]).

## Update 207 (2026-06-11) — IsDecided -1==-1 bug: ~70 two_rows mancala instant-drew on move 1
- Oware (and the whole two_rows family with the "agree-to-end on cycle" rule) ended as an instant DRAW after P1's first move: the end rule (if (or (no Moves Mover) (is Decided "End")) (byScore ...)) fired because (is Decided "End") was spuriously TRUE.
- ROOT: IsDecided.eval returns `state.isDecided() === decisionInt`. The compiler runs NO preprocess pass, so decisionInt stayed UNDEFINED(-1) (registerVoteString never called), and state.isDecided() is unimplemented (also -1) -> -1 === -1 -> true on move 1.
...
## Update 211 (2026-06-11) — Cross-family triage: 3D pyramidal boards + remembered-set sow tracking
- Broadened the survey after the Union/Intersection fix (which helps every `(union A B)`/`(intersection A B)`): board/space now 110/196 OUTCOME_OK.
- NEW SUBSYSTEM NEEDED — 3D pyramidal boards (board/space/connection cluster: Span, Spire, Sponnect, Spaji, etc., plus Pylos-family). `(board (square 4 pyramidal:True) use:Vertex)` builds a STACKED pyramid of vertices (4x4 + 3x3 + 2x2 + 1x1 = 30), with `(is Flat)` gating playable (supported) positions. Our generator makes a flat 4x4=16, so Span's ply-0 `Add to=17` has no such vertex. This is a real 3D-topology feature shared across the Sp* connection games. (Pylos passes — its board is defined differently; compare generators next session.)
- sow forced-pass cases (Adi ply 85, Ako Okwe, Awagagae): Java emits Pass:forced=true because its legal set is EMPTY; we over-generate. Root is upstream — bespoke "can't re-sow from a just-sown hole" tracking via remember/forget Value "P1SowFrom"/"P2SowFrom" over (sites Track) ranges (Adi's "PossibleSowFrom"). Per-game, not a shared forced-pass gap (sequential forced-pass lives in the trial controller, not Game.moves; our harness path differs but the divergence is the over-generation, not missing pass).
- Daramuti (multi-lap, ply 1 residual): Playable remembered set verified intact post-ply-0; issue is the SameTurn multi-lap continuation's from-binding (offers store site 13 only). Per-game.
- Net this window (Updates 201-210): war/leaping ~220/226; sow 52.1% -> 65.0% (277/426); space 110/196; 10 substantive fixes, several cross-cutting (Union/Intersection/NoPieces/IsDecided/Sow-bool). Battery green, units 194/0 throughout.

## Update 212 (2026-06-11) — Forced-pass mechanism VERIFIED working; sow residual is remembered-set forget-range
- IMPORTANT: the forced-pass machinery is fully functional. Awagagae (which my probe script mis-flagged as stuck on the unparseable `Pass:forced=true` line) actually passes 2/2 OUTCOME_OK (ply 1033/613) — it was a MOVE_CAP=600 artifact; our engine generates a proper ActionPass (isPass()=true) and the harness matches it via isPassRecordedMove. So "forced pass" is NOT a remaining gap.
- Adi/Ako Okwe residual ROOT (precise): at Adi ply 85 Java's legal = forced pass (P2SowFrom blocks all of P2's holes), ours = {7,8} because our P2SowFrom=[9,12,10,11] is MISSING 7,8. The `(forget Value "P2SowFrom" (value))` loop over `(sites Track from:(last From) to:("NextHole" (last From) (min 12 (count at:(last From)))))` computes a different swept range than Java — so we forget holes we shouldn't (or never re-remember them). Next session: verify SitesTrack with from:/to: bounds and NextSiteOnTrack distance against the oracle; this `(forget Value name (specific-value))` + Track-range pattern is shared across the PossibleSowFrom/SowFrom mancala (Adi, Ako Okwe, others).
- Pyramidal board (Span/Spire/Sp* connection cluster): RectangleOnSquare stores `pyramidal` but never uses it (builds flat grid). Java appends layers 1..rows-1 at (layer*0.5+col, layer*0.5+row, layer/sqrt(2)) then graph.makeEdges() (3D unit-distance). Our Graph has pushVertex3D but no makeEdges; consumers `(is Flat)` + `(sites Group ... isVisible:True)` also need 3D awareness. Full vertical slice deferred (bounded but multi-part).
- Session totals (Updates 201-212): 5 games cleared 2/2 (Terhuchu, Crand, Laram Wali, Pasang, Oware, Bosh = 6 actually); cross-cutting fixes Union/Intersection 2-arg, NoPieces region/count, IsDecided vote, Sow raw-bool, CountStack per-level, SitesDistance stepMove, Do eval-ctx, SitesOccupied top, ActionMove substack, makeFaces order. sow 52.1%->65%, war/leaping ~220/226, space 110/196. Battery green, units 194/0 throughout. NO regressions shipped.

## Update 213 (2026-06-11) — ForgetValue overload guard (Adi 2/2) + NotAllPass compile flag
- (forget Value "name" (value)) mis-routed to ForgetValueAll: the compiler tries construct* overloads in property order, takes the first non-null, and constructAll accepted the IntFunction arg silently — wiping the WHOLE remembered key per move (Adi's P2SowFrom emptied; the swept-range forget over-forgot family-wide). FIX: constructAll rejects (returns null) unless the discriminating arg is the literal "All"; constructValue rejects "All". Adi 2/2 OUTCOME_OK (ply 3376/1012). SitesTrack from:/to: slicing verified correct against the oracle (range 4->11 exact).
- @java GameType.NotAllPass — Pass.gameFlags (Pass.java:79) and AllPassed.gameFlags (AllPassed.java:71) disable the engine's all-pass-draw fallback; our compiler never set it, so EVERY game with an explicit (move Pass) kept the spurious fallback (Bosh's between-rounds double pass drew at ply 71 where Java plays 342 — Bosh's prior OUTCOME_OK was compensating-bugs luck; it now needs its round-restart dynamics fixed and is an honest MM). Wired via compileFlags.usesExplicitPass (Pass + AllPassed ctors) -> play1to1 stamps game.notAllPass.
- Battery 64/64-equivalent green (Go with the Floe = pre-existing MM matched by a broad filter; Adi 600-cap artifact passes at MOVE_CAP=5000; Adidada WM pre-existing). Units 194/0. sow at 275/426 (Adi +2, Bosh -2 honest, Li'b al-'Aqil/Mangala(Bedouin) flipped — root-cause delegated to a sub-agent).
- PROCESS: parallel Sonnet sub-agents now handle busy work (regression root-cause, sow triage, family sweeps) coordinated from this session.

## Update 214 (2026-06-11) — 3D PYRAMIDAL BOARDS: Span 2/2, Sponnect 2/2, Spaji 1/2 (Shibumi family)
- Subsystem implemented end-to-end, faithful to Java:
  - Graph.findOrAddVertex3D (z-aware dedup — layers project onto base coords) + Graph.makeEdges (@java Graph.makeEdges: join all vertex pairs at 3-D distance ~unit ±0.05) + reorder() preserves z (score stays 2-D y*100+x, stable ties = Java insertion order).
  - makeFaces vertex-major walk now z-GUARDED (@java Graph.makeFaces:1367-73): the m-loop only follows edges whose far endpoint shares the start vertex's plane; pyramid faces form per layer.
  - RectangleOnSquare pyramidal block (@java :105-133): layers 1..rows-1 at (layer*0.5+col, layer*0.5+row, layer/sqrt(2)) + makeEdges.
  - IsFlat ported (@java IsFlat.eval): layer 0 -> true, else every Downward support (edge-neighbour strictly below) occupied. Was a documented always-true 2D stub.
  - SitesGroup isVisible branch (@java SitesGroup.eval): covered-above check (occupied higher-indexed vertex at same (x,y) projection) + >=2 shared occupied UPWARD neighbours blocks the visible link; isVisible:True raw-boolean guarded.
  - SitesGroup CRITICAL fix: BFS expansion called traj.steps with Java's 4-arg signature on our 2-arg API — bound site="Vertex", threw, and SILENTLY fell back to lattice squareNeighbours (wrong on every graph board). Now the 2-arg (site, dir) call. This affects ALL (sites Group ...) games, not just pyramids.
- Span 2/2 (ply 17/13), Sponnect 2/2, Spaji 1/2. Spire/Spought still MM (deeper pyramid mechanics: per-ball visibility variants). Battery green incl Havannah/Y/Gomoku (graph-board canaries), units 194/0.
- SUB-AGENT CORPUS MAP (Sonnet sweep): war 89.3% OK, line 77.7%, hunt 74.3%, blocking 65.8%, territory 41.8%, race 39.2% (!!), group 22.7% (!!). TOP CLUSTERS: (1) race/escape dice games ~32 games systematically MM — single root suspected in stochastic move-gen; (2) hunt fox-and-geese 7 games all ply 1; (3) line ply-0 placement cluster (Andantino, Quarto, Order&Chaos, Teeko...); (4) territory Reversi/Rolit ply 0; (5) war/custodial 7-game flanking cluster; (6) chaturanga checkmate WM cluster. These are the next targets.

## Update 215 (2026-06-11) — Raw-bool traps: ForEachDie.combined + FromTo moveRule/captureRule/fromCondition
- race/escape ply-0 triage (cluster of ~32 dice games): XII Scripta threw `this.combined.eval is not a function` ((forEach Die combined:True)); Pachih threw `this.moveRule.eval is not a function` ((fromTo ... if:True)). Both raw-literal traps — standard typeof guards added (ForEachDie.combined; FromTo fromCondition/moveRule/captureRule). Both games now GENERATE moves (residual divergence is dice-pip-to-distance mapping, deeper per-game).
- Nard's ply-0 "mismatch" has the recorded move (13>18) IN our legal set — the harness-level identity disambiguation (levelMin/levelMax on stacked points) is the divergence, not generation. Filed for the race deep-dive.
- Spot battery green (Backgammon FromTo/ForEachDie paths verified), units 194/0.

## Update 216 (2026-06-11) — SYSTEMIC raw-bool fix: compiler emits BooleanConstant for BooleanFunction slots
- The recurring raw-literal trap (#11) had a single root: ArgCompiler's boolean terminal returned the RAW JS boolean for game.functions.booleans.BooleanFunction-typed parameters, where Java compiles BooleanConstants.True/False (a ludeme with .eval()). Every consumer that didn't defensively typeof-guard threw `.eval is not a function` the first time the slot evaluated (Sow backtracking, ForEachDie combined, FromTo if:, SitesGroup isVisible, ...).
- FIX at the boundary: the compiler now returns {eval:()=>v, isStatic:()=>true} for BooleanFunction slots. Existing ctor typeof-guards stay harmless (an object skips them). java.lang.Boolean slots (stack:, top:, includeSelf: ...) still receive raw booleans — faithful to Java's typed signatures.
- Audit note: 12 move-effect files previously had NO guard (ForEach, ForEachGroup, Apply, FromToFaithful, Note, Step, EffectCtorAdapters, LeapFaithful, Add, Leap, Remember, Do) — all now safe via the boundary fix without touching them.
- Full battery green (Bosh = known honest MM, Update 213), units 194/0.

## Update 217 (2026-06-11) — Sub-agent verification round: naive-probe artifact identified
- VERIFICATION LESSON: the sow-triage agent's stuck-ply numbers are contaminated two ways: (1) the dist was rebuilt mid-flight under it (ForgetValue/NotAllPass/BooleanConstant landed during its run); (2) its probe (copied from my own scripts) matches moves by from/to ONLY, so any game emitting AMBIGUOUS candidates (Daramuti's CCW/CW selects both 5>5) takes the wrong branch and reports a FALSE early divergence. The real harness's chooseMatch (count-delta + lookahead tiers) handles these — Daramuti actually replays to ply 216/48, Kiuthi to 430/439 (not ply 1/2!). Its "values Remembered broken" cluster is REFUTED (all sets verified populated on current build). Standing rule for future agent dispatches: probes must replay via the HARNESS (--verbose, CHOICE_TRACE) or import its matcher; never naive from/to.
- Surviving VERIFIED early targets: Fergen Gobale (ply 0 — P1 offered sites 1-6, rec from=8), Duene (ply 1 — (set Var "From") do-next), Toguz Kumalak (ply 1-2 — tuz/value-Player), O An Quan + Laomuzhu + Ceelkoqyuqkoqiji + Yucebao (ply 3 — largeStack levelFrom subsystem, REAL: recorded Select carries levelFrom=5), Intotoi (ply 2-4), Deka (ply 6 — set State at: + skipIf state).
- Daramuti's "phase 1 after ply 0" was a red herring — Java also advances (unconditional (nextPhase "Sowing") with who=Shared); Java's replayed ply-1 legal is IDENTICAL to ours (13>13) given the same ply-0 pick; the recorded trial just took the CW variant.

## Update 218 (2026-06-11) — SitesBottom/SitesTop: faithful min/max-y primary (Fergen Gobale 2/2)
...
## Update 223 (2026-06-11) — race re-measured (44%); fox-geese relative-direction target pinpointed
- After the raw-bool fixes, race/escape is 77/174 (44%, up from 39%). The remaining race MMs are backgammon/Senet-family per-variant track + custom-throw mechanics (Nard generates 1 move where Java has many — its BackgammonTracksWithBar variant; Senet uses a stick "Throw" + serpentine track) — deep per-variant, no single shared root.
- HUNT fox-and-geese cluster (7 games, all ply 1) PINPOINTED: Jeu Militaire P2 tower {Forwards Rightward Leftward} facing N, at site 0 (neighbours 1,3,4,10; site 1 is due N). Java plays 0->1 (Forwards=N); we generate only 0->3 (Leftward=W). resolveRelativeDir's coneDirections(facing=0, all-8-supported, -3,4) correctly yields {NW,N,NE} in isolation, so the drop is upstream in the Step dirnChoice path (the (directions {Forwards ...} of:...) compilation or facing propagation through ForEach Piece -> component generator). NEXT: trace the live Step.stepTargets for the P2 pawn (the static walk didn't reach its generator); confirm whether dirnChoice pre-resolves facing or Step does, and where N is lost. One fix likely clears all 7 fox-geese games.
- Tree clean, all 22 ledger updates committed.

## Update 224 (2026-06-11) — Component facing overrides player facing in (directions {relatives}) — 7 fox-and-geese games 2/2
- (move Step (directions {Forwards Rightward Leftward}) ...) on a piece DECLARED with its own facing ((piece "Pawn" P2 N ...)) resolved the relatives against the PLAYER default (P2=S) instead of the component's N facing: dirnChoice.eval@site0 returned [SE,S,SW,W,E] (S-arc) so the real Forwards=N step to the adjacent N site was never generated.
- ROOT: compilePreferredDirectionsRelative passed facingOverride=undefined to resolveRelativeDir, so it used (mover===1?0:4). @java Component.getDirn() overrides the player facing. FIX: read componentFacing[what(_evalFrom)] (the COMPASS8 token) and pass it as the override — exactly the logic Step.stepTargets already had for the single-dir path; the (directions {...}) compiler path lacked it.
- Cleared 2/2: Jeu Militaire, Gasetavl, Gasetavl (Gedved), Jeu de Renard, Jeu de Renard (Two Foxes), Renard et les Poules, Wolf und Schaaf. hunt family 113 -> 129/152 OUTCOME_OK.
- Battery green (Loop Xiangqi/MiniXiangqi/Xiangqi MM+WM are PRE-EXISTING — verified by stash-revert), units 194/0.

## Update 225 (2026-06-11) — Ahead raw-string direction (start placements (ahead (centrePoint) DIR))
- Shi Liu Kan Tsiang Kun placed NO General: (place "General1" (ahead (centrePoint) S)) returned the centre itself (17), and a Marker then overwrote it. ROOT: an absolute lud direction (S/N/...) reaches Ahead as a RAW STRING (raw-literal trap), not a DirectionsFunction — Ahead checked .name/.convertToAbsolute (both undefined on a string) and fell through to `return site`. The SameDirection/OppositeDirection raw strings were already handled; plain compass strings were not.
- FIX: when dirnChoice is a raw non-relative string, use it directly as the direction name. radialsByName(17,"S")=[[17,12,7,2]] then steps to 12 correctly.
- Affects 9 games using (ahead (centrePoint) DIR) start placements (Shi Liu, Ringo, ...). Shi Liu General now placed + generates moves (residual ply-1 P2-Marker issue separate); Ringo start fixed (boardless residual). Fanorona (ahead SameDirection) unaffected — no regression. Battery green, units 194/0.

## Update 226 (2026-06-11) — Flip robustness (no longer aborts move lists); flips subsystem mapped
- Reversi/Rolit threw `fc.containerId is not a function` inside Custodial->Apply->AllCombinations->Flip, aborting the ENTIRE ply-0 move list (0 legal moves). The escape-hatch methods (containerId/containerState) are absent on the nested context (and even the main one). FIX: Flip falls back to the single board container (cid 0) read off ctx.state, and guards components()/getFlips — Java never throws there. Reversi now GENERATES moves (no crash); custodial canaries (Tablut/Hnefatafl) + chess unaffected, units 194/0.
- Reversi/Rolit still MM at ply 0 — the FLIPS SUBSYSTEM is multi-part and deferred: (piece "Disc" Neutral (flips 1 2)) compiles to a PLAIN Object, not a PieceFaithful, so getFlips() is undefined and no actual flipping happens. NEXT: route piece compilation through the Piece class so the Flips arg (slot 4, game.util.moves.Flips — registered) binds and getFlips() works; then Flip's flat path produces real ActionSetState flips. Affects Reversi, Rolit, MacBeth, and other (flips ...) territory games.

## Update 227 (2026-06-11) — Extended session final consolidation (Updates 201-226, 27 commits)
GAMES CLEARED 2/2 this session (~25): Terhuchu(proper), Crand, Laram Wali, Pasang, Oware, Adi, Awagagae, Fergen Gobale, Span, Sponnect, Odd, Order and Chaos, Quarto, Tic-Tac-Chess, Jeu Militaire, Gasetavl, Gasetavl(Gedved), Jeu de Renard, Jeu de Renard(Two Foxes), Renard et les Poules, Wolf und Schaaf — plus the two_rows mancala family (52%->67%) and partial advances (Bosh/Daramuti/Teeko/Shi Liu/Reversi no longer crash).

CURRENT FAMILY STANDINGS (MOVE_CAP=5000): hunt 129/152 (85%), line 232/292 (80%), sow 285/426 (67%), blocking ~66%, race/escape 77/174 (44%), territory ~42%, group 18/66 (27%); war ~89%.

CROSS-CUTTING FIXES (all faithful @java, battery + units 194/0 before every commit): IsDecided vote bug, systemic BooleanConstant at compiler boundary, Union/Intersection 2-arg ctors, NoPieces region/count, ForgetValue overload guard, NotAllPass flag, SitesBottom/Top min/max-y, SitesHand Shared + copy:True + container:(IntFunction), 3D pyramidal subsystem (+SitesGroup graph-board steps), makeFaces vertex-major order, CountStack per-level, SitesDistance stepMove, component-facing in (directions {relatives}) [7 fox-geese], Ahead raw-string direction [9 games], Flip board-container robustness.

DEFERRED MULTI-PART SUBSYSTEMS (each well-mapped, ~3-10 games): flips (Reversi/Rolit/MacBeth — Piece compiles to plain Object, getFlips lost); race dice-pip/track per-variant (backgammon/Senet families); largeStack mancala (pits-as-stacks); boardless dynamic boards (Andantino/Ringo); group bespoke scoring; is-Pattern win-detector (Teeko); dual-SiteType pyramidal moves (Spire); multi-lap relay-sow tails. Task #42 (upstream Java-bug round-trip) remains the end-of-campaign step.

## Update 228 (2026-06-11) — Flips subsystem works: Reversi plays full game (flip data + flat sizeStack)
- TWO fixes made the (flips A B) flip mechanic functional:
...
## Update 230 (2026-06-11) — territory 42%->54%; flips subsystem DONE; standings
- territory now 53/98 (54%, was 41.8%) from the flips + SitesState + Append-then fixes. Reversi/MacBeth cleared; Rolit advanced ply 0->60 (endgame board-fill residual, Rolit-specific). The territory WINNER_MISMATCH residue (Flume/Goats Wintering/Stargazers/Phantom Go/...) is diverse per-game scoring (byScore+count Sites, no-Moves, group counts) — not one shared bug.
- SESSION CUMULATIVE (Updates 201-229, 30 commits, 0 regressions): ~28 games cleared 2/2; families lifted — hunt 85%, line 80%, territory 54%, sow 67%, war ~89%, blocking ~66%, race/escape 44%, group 27%. Three deferred frontiers now COMPLETE/major-progress: flips (Reversi family), 3D pyramidal (Span/Sponnect), fox-and-geese facing (7 games).
- REMAINING (each a focused subsystem): race dice-pip/track per-variant (~50 games, the biggest frontier), group bespoke scoring, largeStack mancala, boardless (Andantino/Ringo), is-Pattern (Teeko), multi-lap relay-sow tails, per-game territory scoring. Task #42 (upstream Java-bug round-trip) remains the end-of-campaign step.

## Update 231 (2026-06-11) — Race frontier root-caused: (max Distance ...) over-filters (NOT a dice bug)
- Investigated Nard (race/escape, generates 1 move where Java has ~6). RULED OUT dice: Java Die.roll returns context.rng().nextInt(faces.length) — the INDEX — so recorded SetStateAndUpdateDice state = the face index; our pip values come out equivalent. Confirmed by injecting dice directly: move-gen then yields the correct set including the recorded 13>18.
- TRUE ROOT: (play (RollEachNewTurnMove (priority {("EnterPieces") (max Distance (forEach Piece top:True))}))). Our `(max Distance ...)` keeps only the single LONGEST move (4>14, both dice combined), filtering out 13>18 (one die). Java's `max Distance` is the backgammon must-use-both-dice rule: it keeps every move that participates in a maximum-distance SEQUENCE over the turn (its fresh roll legal set is mixed-pip: 13>16, 13>14, 7>3, 7>5, 4>1, 4>3 — single-die moves of differing length, all legal). So our Max/MaxDistance requirement ludeme over-filters.
- This is the MaxMoves/MaxDistance subsystem (Backgammon canary passes — its recorded trials likely don't expose the divergence). Faithful fix needs Max.eval to compute the per-turn max-distance reachability (2-ply: which first-moves leave a legal second move) rather than max single-move distance. Affects the backgammon/race family broadly. Deferred as a focused subsystem with this precise entry point.

## Update 232 (2026-06-11) — MaxDistance per-turn recursion (race family advances deeply)
- Ported MaxDistance.getDistanceCount (@java MaxDistance.java:143-190): the 2-ply/per-turn lookahead. Apply the candidate move; if the SAME player still moves (a die remains — backgammon plays both dice in one turn), recurse over the resulting legal moves accumulating track distance; keep every move on a MAX-total-distance turn sequence. Previously a stub (return single distance) that kept only the longest single move — Nard offered 1 of ~6 moves.
- newCtx carries `recursiveCalled` (the outer eval already checks it) so the nested moves() uses single-move distances; this method drives the recursion, bounding depth to the dice count.
- Nard ply 0 -> 199; Plakoto/Senet/Pachisi all advance from early to mid/late plies. Backgammon canary still 2/2 (no regression), units 194/0. race/escape 77 -> 79/174 (most newly-advanced games have deeper per-variant residuals: bear-off, hit-capture, custom tracks).
- The MaxDistance subsystem is now correct; remaining race work is per-variant track/bear-off nuances.

## Update 233 (2026-06-11) — SitesPlayable boardless branch; boardless board-SIZING is the real blocker
...
## Update 239 (2026-06-11) — Global parity estimate + crash-class cleared
- Full-corpus sweep (2304 trials, MOVE_CAP=5000): the 400-trial running tally was COMPILE_FAIL:0 START_FAIL:2 MOVE_MISMATCH:137 WINNER_MISMATCH:14 OUTCOME_OK:247 = ~62% OUTCOME_OK (early-alphabetical board games). Combined with per-family measurements (war ~89%, hunt 85%, line 80%, sow 67%, blocking ~66%, territory 54%+, race/escape 45%+, group 27%, connection lifted by pyramidal/SitesGroup), the corpus sits around ~60-65% OUTCOME_OK at full move cap — up from a much lower baseline at this campaign window's start.
- Crash class (throws at start/move-gen) is now 18/1150 distinct games, almost all distinct subsystems: (match ...) multi-game files (6), topology edge<->cell incidence completeness (2-3), Union ctor arity for the 30k-line Throngs (2), high-player-count component generation, and a few ctor-dispatch singletons. The broad crash bugs (Region .sites(), PlaceRandom containerId, SitesIncident iteration) are fixed.
- This session (Updates 201-239): ~40 games cleared 2/2; 10+ subsystems completed or advanced (flips, 3D pyramidal, fox-geese facing, MaxDistance per-turn recursion, two_rows mancala family, shared-hand, boardless, SitesState/Append scoring, PlaceRandom/Region/Incident crash guards). All faithful @java ports, battery + 194 units green throughout, zero regressions shipped.

## Update 240 (2026-06-11) — Topology edge/vertex .cells incidence getters (SitesIncident correctness)
- SitesIncident read edge.cells / vertex.cells (Java's Edge.cells()/Vertex.cells() = incident FACES), but our GraphTopology stored them as edge.faces / vertex.faces — so those reads were undefined (guarded to [] in Update 238, but that lost real incidence). Added `get cells()` aliasing `faces` on EdgeEl and VertexEl. (sites Incident) over edge/vertex now returns the true incident cells.
- Triple Tangle advanced (ply 0->1) but still MM — it's an EDGE-PLAY game (pieces ON edges, typeFrom=Edge moves) needing the edge-play movement subsystem (separate). Nodal Chess similar. ConHex ply-0 MM is pre-existing (special ConHex board). Guerrilla Checkers (incidence canary) stays 2/2 — no regression. Battery green, units 194/0.
- The incidence reads are now correct for the whole (sites Incident) family; remaining incidence games need edge-play movement or special-board generation.

## Update 241 (2026-06-11) — Null-direction crash hardening (toAbsolute guards across 6 files)
...
## Update 246 (2026-06-11) — Surround silent dropped-then fixed; dropped-then audit complete
- Audit for SILENT dropped-then (ludemes storing then in super but never attaching it to their moves, distinct from the explicit `void then;`): Surround built its capture moves without applying its (then ...) consequence. Wired as a deferredThen. Faithful (@java Effect.then), no regression (Havannah/Span/Tablut/Oware/Chess green; Pylos MM is pre-existing, advanced ply 0->11 from earlier fixes), units 194/0.
- Deal (card/domino dealing — rare, mostly start-of-game) also silently drops its then; flagged low-priority. Swap delegates then to its sub-ctor. The "Faithful" wrappers (RemoveFaithful/AddFaithful/HopFaithful/...) correctly pass then to their real ludeme. Dropped-then class (explicit + silent) now substantially closed: Append, SetScore, SetVar, AddScore, Attract, Directional, Push, Surround fixed.

## Update 247 (2026-06-11) — (count Groups <dir>) honoured its connection direction
- CountGroups hardcoded "Adjacent" (8-way on square boards) and Count.constructGroups dropped the Direction arg entirely. (count Groups Orthogonal …) thus merged diagonally-touching pieces, under-counting groups; SingleGroupWin's (= 1 (count Groups …)) fired at ply 1. Threaded the direction (default Adjacent @java CountGroups dirnChoice) and walk traj.group(s, dir). Direction token arrives RAW as a bare-enum string (raw-literal trap) — coerce string|{name}. Same coercion applied to Sites.constructGroup (SitesGroup).
- Result: Groups + Bounce now OUTCOME_OK (were WINNER_MISMATCH ply 1). No regression (Lines of Action/Manalath/Yoin/Signum green), units 194/0. Spao/Elea remain WM at later plies (distinct group-scoring bug).

## Update 248 (2026-06-11) — ForEachGroup: three faithful fixes unlock group-scoring then-clauses (Elea + group cluster)
Three independent breaks in ForEachGroup.eval, all silent (its exception was swallowed upstream, so group-scoring then-clauses produced NO score moves):
1. `owned.sites(i)` — that method does not exist on the TS Owned surface (it has positions(pid)). The call threw. Replaced with positions(pid) → union of site()s, the faithful equivalent of Java owned().sites(pid).
2. `this.type` null was passed to getGraphElements(null) → size 0 → maxIndexElement 0 → the `site < maxIndexElement` filter dropped EVERY owned site. Resolve null to board.defaultSite() (Java resolves in preprocess). Also applied to cs.who(...).
3. Neighbour walk called the 2-arg Trajectories.steps with 4 args (shifted: dir became a number → []), AND hardcoded "Adjacent" ignoring dirnChoice. Use traj.group(site, directionName) with the resolved connection direction (default Adjacent), raw-string coerced like CountGroups.
Result: Elea now OUTCOME_OK (was WINNER_MISMATCH ply 1/61). No regression: Bide/Meridians/Lifeline/Archworm were MM at identical plies BEFORE the change (verified by stash A/B); Mono/Odd/Groups/Bounce still/now OK; units 194/0. Spao/Brood/Manifold remain WM — they score via (sizes Group) / shape-vector states, a separate path (still deferred).

## Update 249 (2026-06-11) — SizesGroup: same 4-arg steps bug fixed ((sizes Group))
SizesGroup._neighbours called the 2-arg Trajectories.steps with 4 args (steps(type, site, type, dir)) — args shifted, directionByName saw a number, returned [], so every group collapsed to a singleton on graph boards (wrong (sizes Group) sizes everywhere). Replaced with traj.group(site, directions). No regression (Sibling/Catchup/Omega/Product MM at identical plies before/after by A/B stash), units 194/0. Spao remains WM: it is a Shibumi 3D pyramid needing vertical "hidden connections" — the deferred pyramidal-connectivity case, beyond this singleton bug.

## Update 250 (2026-06-11) — Trajectories: Java-shaped 4-arg steps(type,from,toType,dir) overload (closes the arg-shift bug class)
Root-caused a systematic latent bug: six ported ludemes (SitesSupport, CountSitesPlatformBelow, SitesLoop, All, ForEachDirection, SizeTerritory) call topology.trajectories().steps(type, fromIdx, toType, dir) — the Java signature — but the TS Trajectories class only had a 2-arg steps(site, dir). JS silently dropped the extra args, so `dir` became a number, directionByName failed, [] was returned, and `.to().id()` was never reached (empty result, no crash). These features degraded to silent no-ops. Added the faithful 4-arg overload returning Java-shaped Step objects ({to():{id()}}) alongside the preserved 2-arg path (extracted to stepsTwoArg). Regression-free: BEFORE/AFTER identical across Go/Havannah/Hex/Span/Spirit/Splastwo/Spire and SizeTerritory variants (Walking Dots/Patok/Lotus/One-Eyed Go OK); units 194/0. Visible payoff for the 3D Shibumi callers (Downward/UNW/USW steps) awaits wiring those pyramidal directions into the core trajectories — deferred 3D-connectivity work — but the signature bug is now closed for all standard-direction callers.

## Update 251 (2026-06-11) — Enclose: port the missing atLeastAnEmpty single-group boundary check (Mig Mang ply 1 -> 134+)
Enclose.eval was missing Java's atLeastAnEmpty branch entirely. When the between/target rule can match EMPTY sites (e.g. Mig Mang's (or enemy empty)), empties join the enclosed region, so the ordinary "empty neighbour = liberty" test never fires and a single stone "encloses" the whole board — at ply 0 Mig Mang removed 6 pieces and set state=1 on 288 of 289 sites, leaving 1 legal move.
Faithful port (@java Enclose.java:140,237-290):
- atLeastAnEmpty: Java reads targetRule.concepts(game).get(IsEmpty); the concept system isn't on the TS BooleanFunction, so compute the equivalent at runtime — the rule "can match empty" iff isTarget is true at some empty site.
- Collect the enclosing boundary (enclosingGroup) during the liberty scan; liberty test becomes `atLeastAnEmpty ? isTarget(to) : what(to)==0`.
- When atLeastAnEmpty, the region is enclosed ONLY if the boundary is a SINGLE connected group (walk it via All-direction adjacency, consuming the list).
Result: Mig Mang advances ply 1 -> 134+ (no spurious capture/state at ply 0; 276 legal moves). No regression: Go/NoGo/One-Eyed Go/Lotus/Walking Dots all still OUTCOME_OK (their enemy-only target → atLeastAnEmpty=false → original path), units 194/0.

## Update 252 (2026-06-11) — SizesGroup: port isVisible (3-D Shibumi visibility); 3-D trajectory infra confirmed working
Verified the 3-D pyramidal trajectory infrastructure is actually WIRED: for Spao (30-vertex pyramid, layers 16/9/4/1), traj.steps(Vertex, v, Vertex, "Downward") returns the 4 supports and group(v,"Adjacent") includes inter-layer neighbours. The mapAngleToAbsoluteDirection mapper already classifies steps as U*/D* by z-delta and COMPASS_UPWARD/DOWNWARD aggregate them. So the Shibumi blocker is NOT the graph.
SizesGroup.eval had an isVisibleFn field but IGNORED it. Ported the Java isVisible logic (@java SizesGroup.java:153-243), mirroring the already-working SitesGroup: a seed/neighbour hidden by a higher-indexed occupied vertex at the same (x,y) is skipped (covered), and a link blocked by >= 2 SHARED occupied upward supports is skipped. Regression-safe: gated on isVisibleFn (null for all flat games → isVis=false → unchanged); flat (sizes Group) games (Catchup/Omega/Product) identical, units 194/0.
Spao still WINNER_MISMATCH at ply 30 (the byScore end) — the isVisible port is necessary but not sufficient; the remaining group-product divergence needs a Java-oracle position comparison (next step). The port is faithful and required for parity regardless.

## Update 253 (2026-06-11) — Mul single-arg slot + SizesGroup role/min defaults: group-PRODUCT scoring cluster fixed (Spao/Catchup/Omega/Product)
Root-caused the group-product scoring family via the recorded trial as a Java oracle (its SetScore actions embed Java's per-move scores). Three chained bugs in `(* (sizes Group <role>))`:
1. **Mul**: `(* (sizes …))` (a lone array arg) was delivered by the reflection compiler in the SECOND ctor slot — Mul(null, array) — so the two-value branch ran `null.eval` and threw. Java funnels both ctor forms into the array, so made Mul position-agnostic: null/undefined in either slot → single-array form.
2. **SizesGroup whoFn**: a bare role (Mover) arrived RAW as a string, not an IntFunction (raw-literal trap). Coerce via roleToIntFunction (@java RoleType.toIntFunction); All/Shared/missing → allPieces.
...
## Update 255 (2026-06-11) — (sites Occupied by:(player <fn>)) resolved correctly (Verge); SitesOccupied neutral guard
Root-caused Verge (territory, WM→OK) with the recorded-trial oracle: it has no scores; `(if (no Moves Next) (result Next Win))`, and TS over-generated 29 terminal moves (all empty orthogonal neighbours of P2's frozen groups) where Java had 0, so the end never fired. Chain: the play excludes `(sites Around (FrozenGroups (mover)) Orthogonal)`; FrozenGroups = `(forEach (sites Occupied by:(player #1)) if:…)`; that SitesOccupied returned 0 because `resolveIntFn` mishandled `(player <fn>)`.
- **resolveIntFn (Sites.ts)**: `game.util.moves.Player.index()` returns an IntFunction (not a raw index). The old code wrapped it as `{eval:()=>idxFn}`, so `who.eval` returned the function OBJECT ([object Object]); `owner===whoId` never matched → empty region. Now: if `index()` yields an eval-able, return it directly; if a number, wrap it. Faithful to Java (by = player.index()). Fixes Verge; Atom + broad canary (Go/Tablut/LoA/Oware/Fox&Geese/Hex) all still OUTCOME_OK; units 194/0.
- **SitesOccupied neutral guard**: with `(player <fn>)` now resolving, `by:(player 0)` would hit the default branch and match every EMPTY cell (owner 0). Guard: default branch requires whoId>0 OR an occupied cell (@java owned().sites(p) lists occupied sites). 
KNOWN ply-shift (not a passing-game regression): Stargazers (was 0/2: MM37+WM35) now 0/2 at ply 0, and Vigilance (already failing) — both exercise `(sites Occupied by:(player #1))` in setup/early play and now expose a separate bug (Stargazers' mover-0 neutral star placement) that the previously-empty region masked. Net this update: +2 passing trials (Verge), 0 passing trials lost.

## Update 256 (2026-06-11) — (sites Around … Empty/NotEmpty …) tests occupancy, not ownership (Flume)
Oracle (recorded-trial) on Flume: all 49 placement SITES matched but movers diverged (TS gave a player an extra turn) — its `(then … (if (< 2 (count Sites in:(sites Around (last To) NotEmpty Orthogonal))) (moveAgain)))` mis-counted neighbours, desyncing turns. Root cause: dynamicRegionAccepts classified Empty/NotEmpty by OWNER (`who !== 0`), but Flume's Neutral outer-ring discs have owner 0 and a non-zero component — they were wrongly counted as empty. Fixed to test occupancy (`who !== 0 || what !== 0`). For owned pieces who!=0 ⟺ what!=0, so only the neutral case changes — strictly more correct, no regression (Go/Tablut green, units 194/0). Flume now OUTCOME_OK (P2 28 > P1 21, matches Java winner=2). Affects any game with neutral pieces + Around Empty/NotEmpty filters.

## Update 257 (2026-06-11) — Do prepend shifts decisionIndex (Goats Wintering; general do/next fix)
Oracle on Goats Wintering: all moves+scores matched (TS [P1=2,P2=1]) but game.over=false, ctx.winner=-1 → WM. Its `(do (set Var "LT" …) next:(move Pass (then (set Var "LM" 0))))` produced a move whose actions were [SetVar,SetVar,Pass,SetVar] with the Pass NOT flagged isDecision, so Move.decisionAction() fell back to actions[0] (a SetVar) → isPass()=false → (all Passed) never fired → no termination. Do.eval prepends the prior's actions but had not adjusted decisionIndex. Set `decisionIndex = priorActions.length + nm.decisionIndex` so decisionAction() resolves to the NEXT move's decision (the Pass). General fix for all (do <prefix> next:<decision>) composites. Goats Wintering now OUTCOME_OK (winner=P1, ranking [0,1,2] match Java). No regression: Backgammon/Kalah/Chess/Chessense (heavy (do (roll) next:…) users) + Go/Tablut/Atom/Oware/Flume all OUTCOME_OK; units 194/0.

## Update 258 (2026-06-11) — IsLine roleOwner handles Player (Gobblet Gobblers; mixed-size lines)
Oracle on Gobblet Gobblers: all moves matched but game.over=false (no winner). Its end `(forEach Player if:(is Line 3 Player top:True) (result Player Win))` — IsLine.roleOwner("Player") returned null, so targetWhats fell through to the pivot's SINGLE component. Gobblet's winning column (sites 1,4,7) holds different-size pieces (components 3,5,3) of the same owner, so the mixed-size line went undetected and the game never ended. Added the Player case (→ _evalPlayer, the forEach-Player iterator). Gobblet Gobblers now OUTCOME_OK; Tic-Tac-Toe/3D Tic-Tac-Toe/Misère/Tapatan/Achi unchanged (single-piece players: the player's component set is a superset of the pivot's, so no behavior change); units 194/0.

## Update 259 (2026-06-11) — Flip defers its (then …) so it sees the POST-flip state (Minefield)
Oracle on Minefield (Othello-on-states): at ply 64 TS over-captured — flipped the 4 neighbours of the selected cell and added a Disc there with state=mover, while Java changed only the selected cell. Cause: Flip.eval evaluated its thenClause EAGERLY (this.thenClause.eval(ctx) at move-generation, then withConsequence), so the nested `(flip (last To) (then ("ReverseBoundedPieces")))`'s state-keyed custodial read the PRE-flip state(20)=1 instead of the post-flip 2, matching the wrong run. Fixed: attach the then as a deferredThen so evalDeferredThens runs it against the post-flip state (like every other deferred-then ludeme). Minefield now OUTCOME_OK (was no-termination WM). No regression: Reversi/Rolit (flip-capture games) still OUTCOME_OK, units 194/0.

## Update 260 (2026-06-11) — replay harness disambiguates same-from/to ADD variants (Pentago)
Oracle on Pentago: the ENGINE is correct — for a quadrant rotation it generates BOTH directions (CW re-adds D5→E6, CCW re-adds D5→E4), and the CCW variant exactly matches Java (remove 27, add 22). Both Select moves share from/to=28 (the quadrant centre), so the replay harness's chooseMatch picked the wrong one → board diverged at ply 3 → MM. This is a TEST-HARNESS matching gap, not a port bug (the engine/reflection code is untouched). Added a disambiguation tier (alongside the existing state/promotion/dice/capture-direction tiers): when candidates share from/to, apply each and prefer the one whose resulting board reaches the recorded Add:to=N,what=W cells. Pentago now OUTCOME_OK; Chess/Backgammon/Quarto/Quantik (other multi-candidate games) unaffected, units 194/0.

## Update 261 (2026-06-11) — Ahead resolves a (directions Cell from:X to:Y) DirectionsFunction (Boop)
Oracle on Boop: at ply 3 TS REMOVED a booped piece that Java SLID. Boop's ("Repel") uses `(if (= (ahead (site) steps:1 (directions Cell from:(last To) to:(site))) (site)) (remove (site)) (slide …))` — Ahead returns the site itself when it cannot resolve the direction, which Repel reads as "edge → remove". Ahead handled SameDirection/raw-string/convertToAbsolute/name directions but NOT a plain DirectionsFunction exposing only eval(ctx) (which `(directions Cell from:X to:Y)` is) — it fell through to `return site`. It coincidentally matched for genuinely off-board diagonals (NW at the top edge) but wrongly removed interior diagonals. Added an eval() fallback. No regression: Tablut/Fanorona/Fox and Geese/Go/Tic-Tac-Toe/Oware all OUTCOME_OK, units 194/0. Boop now SLIDES (not removes); its remaining divergence is the slide direction ((slide … (directions Cell from:(last To) to:(from))) — the (from) binding during slide eval), documented for the next pass.

## Update 262 (2026-06-11) — Slide honours a dynamic (directions Cell from:X to:Y) (Boop repel)
SlideFaithful collapsed its direction to a static name, passing null (→ default "Adjacent") whenever the direction was a DirectionsFunction. Boop's ("Repel") `(slide (from (site)) (directions Cell from:(last To) to:(from)))` thus slid the booped piece in the wrong direction (all-adjacent instead of the specific away-from-placement vector). Added a dynamic `dirnFn` to Slide, evaluated per-from at eval time (after ctx._evalFrom is bound) and used as absolute compass names directly (skipping the mover-facing relative resolution). With the Ahead fix (Update 261) Boop's board now matches Java through ply 16 (the boop pushes are correct); its remaining ply-17 divergence is a separate promotion-move layer. No regression: Tablut/Chess/Chessense OUTCOME_OK, units 194/0.

## Update 263 (2026-06-11) — IsLine exact:True considers the FULL line (both rays), matching Java
The traj (graph-board) path of IsLine.evalFlat had a standalone `if (matchesLen(count)) return true` after the opposites loop, so with exact:True it matched a single forward ray of exactly `len` even when the opposite ray extended the line beyond `len` (a 3-in-a-row that is really part of a 4-line is NOT a line of exactly 3). @java IsLine.eval only returns in the forward walk when !exact; for exact it checks the FULL oppositeCount (forward+opposite) == len at the end. Restructured to mirror the square path's else-branch: when opposites exist, only the combined count is tested; the single-ray check applies only when there are no opposites. Regression-free across all mill/line games (Three/Nine/Twelve Men's Morris, Picaria, Tapatan, Pentago, Tic-Tac-Toe, Gomoku, Connect Four, Hex) and units 194/0. (Altan/Dala remain MM — their divergence is a separate line UNDER-detection / capture-scoring layer, not the exact over-match.)

## Update 264 (2026-06-11) — IsLine exact:True now honoured (BooleanConstant, not raw boolean) — Altan + Dala fixed
Probe showed `(is Line 3 Orthogonal exact:True)` evaluated with exact=FALSE. IsLine's ctor used `constBool(typeof exact === "boolean" ? exact : false)`, but the compiler delivers `True` as a BooleanConstant OBJECT (the raw-literal wrapping rule), so the typeof check failed and exact silently defaulted to false → `(is Line 3 …)` matched count>=3, firing on 4-in-a-rows. On the Alquerque board (all edges Orthogonal, so diagonals count as orthogonal lines) Altan/Dala over-fired their mill `(moveAgain)` on diagonal 4-lines. Fix: use boolFn(exact, false) (handles BooleanConstant + raw boolean + null), like every other IsLine bool param. Combined with Update 263 (full-line exact check), Altan Xaraacaj and Dala now OUTCOME_OK (2/2, were MM 92/15). Regression-free: NMM/Three/Twelve Men's Morris, Tic-Tac-Toe(+Misère), Gomoku, Pentago, Picaria, Tapatan all OUTCOME_OK; units 194/0.

## Update 265 (2026-06-18) — radialsForDirection matches axes by GEOMETRY (width), not fixed index — Push direction (Quixo/Tara)
radialsForDirection assumed axes were ordered [EW, NS, NESW, NWSE] (buildFlatRadials), but the graph-derived _radials attached for square Cell boards store axes in DISCOVERY order. For Quixo's corner site 0, axes[0] is the COLUMN, so a push with direction "E" walked the wrong line — Quixo's board diverged at ply 26 (push down the column instead of across the row). Added an optional width param: for a single compass direction (E/W/N/S/NE/…), pick the axis whose ray[0]→ray[1] step matches the requested (dx,dy), orienting via opposite. Threaded width from Push. Result: Quixo's board now matches Java for 42 plies (was 25); Tara gains a passing trial; Pushing Me XO still OUTCOME_OK; units 194/0. Quixo's remaining ply-43 divergence is a separate selection-filter case. (Backward-compatible: without width, the index lookups are unchanged, so Attract/Hop/Directional/Step are untouched for now.)

## Update 266 (2026-06-18) — Push preserves the shifted piece's component owner (Quixo + Tara)
Push's ActionAdd forced owner=mover on every shifted piece, so a pushed NEUTRAL Square (and any pushed enemy piece) became the pusher's. Quixo's selection `(from (sites Outer) if:(or (is Mover (who at:(from))) (= (who at:(from)) 0)))` then mis-saw a shifted Square at site 2 as owned by P1 (who=1) instead of neutral (who=0), so P2 could not take it and the replay diverged. @java Push.java's ActionAdd derives the owner from the COMPONENT (Disc→P1, Cross→P2, Square→Neutral), not the mover. Fixed: owner = ctx.components()[currentPiece].owner. With Update 265's radial-geometry fix, Quixo and Tara are now OUTCOME_OK (2/2, were MM/WM); Pushing Me XO still OUTCOME_OK; units 194/0.

## Update 267 (2026-06-18) — Remove honours its level: (remove X level:0) trims one stack level, not the whole pile (Complica)
Oracle (action-matching) on Complica: all 26 moves applied correctly, but at ply 23 a full column (size 8) was emptied (size→0) instead of trimmed to 7. The then `(if (= (size Stack at:(last To)) 8) (move Remove (last To) level:0))` removes the BOTTOM level, but Remove.ts dropped the levelFn (built ActionRemove with just {to}), and ActionRemove.apply had no level branch — so it cleared the whole stacked site. Fixes: (1) Remove.ts passes level to ActionRemove; (2) ActionRemove.apply, when given an explicit level on a multi-piece stack, uses withStackPop(level) (splices that level + shifts what/value columns), with FullOwned registry maintenance. Gated on level>=0 AND a real stack so the default level-less remove is unchanged. Complica now OUTCOME_OK (2/2, was WM); Backgammon/Lasca (stacking) unaffected; units 194/0. Shared fix for any (remove … level:N).

## Update 268 (2026-06-18) — `container:0` is the BOARD container, not the hand (Boop)
The compiler's `(sites Occupied … container:<x>)` intercept coerced ANY non-string `container:` arg to the string `"Hand"` (a hack originally added for Teeko's `container:(mover)` hand-placement). But `container:0` is the **board** container (index 0): Boop's `IsFullBoard = (= 8 (count Pieces in:(sites Occupied by:Mover container:0)))` counts the mover's 8 BOARD pieces to trigger the full-board promotion `(moveAgain)`. Routing it to the hand made the region always empty → count 0 → `IsFullBoard` never true → the promotion `(moveAgain)` never fired, so after a player placed their 8th piece TS advanced to the opponent instead of staying to promote (the recorded Java move was `Select 13 / Remove 13 / SetScore +1`). @java SitesOccupied's `container` is an `IntFunction` container INDEX; index 0 is always the board. Fix (ArgCompiler `occupied` branch): a numeric-literal `container:0` → `null` (board scan); the int-function form (`container:(mover)`) still routes to the hand scan, and the string form (`container:"Hand"`/`"Board"`) is unchanged. Boop is now OUTCOME_OK (2/2, was MOVE_MISMATCH); A/B-verified the other 4 `container:0` luds (Kos, Santorini, Quon) are UNCHANGED (pre-existing WINNER_MISMATCH/START_FAIL, not regressions); Quixo/Tara/Complica/Nine Men/Teeko/Shogi unaffected; units 194/0. Boop was a long-standing deferred game — now closed.

## Update 269 (2026-06-18) — DIAGNOSED + DEFERRED: chaturanga/shatranj false-checkmate (RoleType.Next=0 in the end-rule context)
Root cause FOUND for the chaturanga/shatranj/xiangqi WINNER_MISMATCH cluster (Saxun, Shatranj at-Tamma, Tepong, Krida Buddhibalasrita, Chaturanga 14x14, etc.): these games declare the winner via the `"Checkmate"` def
  `(if (and ("IsInCheck" K Next) (not (can Move (do (forEach Piece Next) ifAfterwards:(not ("IsInCheck" K Next)))))) (result Mover Win))`.
The end rule is evaluated in `evalCtx` built from the post-move state, where our `State.next` is **0** (it is only set by an explicit `(set NextPlayer)` and is otherwise cleared — Game.ts Step 6 `withNext(0)`). `RoleType.Next` in ForEachPiece reads `state.next` → resolves to player **0**, so `(forEach Piece Next)` iterates a non-existent player, finds no escape move, and a FALSE checkmate fires the instant the opponent is in check (the games ended mid-game instead of playing on to the turn-limit draw recorded by Java; `winner=0 endtype=TurnLimit`). Confirmed empirically: `[FEP] specificPlayer=0 stateMover=1 role=Next movesGenerated=0`.

Two faithful-looking fixes were each PROVEN to regress a passing game, so BOTH were reverted (no net regression shipped):
1. Recompute `Next` from the mover in ForEachPiece's `roleToIntFunction` — fixed Saxun (2 WM→OK) + partially Tepong/Krida/at-Tamma, BUT regressed Shatranj al-Mustatila (OK→MOVE_MISMATCH). Reason: `IsThreatened` switches `state.mover` to the enemy but (per @java IsThreatened.java) preserves `state.next()`; recomputing Next from the switched mover corrupts the `(is Threatened K (forEach Piece Next))` idiom.
2. Stamp the implied next onto the END-EVAL state only — fixed the same set, but STILL regressed Shatranj al-Mustatila. Reason: that game is a DICE shatranj (`(do (roll) …)`); activating opponent move generation inside the end-rule eval consumes RNG during *generation*, desyncing the recorded die roll at the next ply (`tsMoveCount=1` = wrong die value).

The correct faithful fix requires maintaining `State.next` as Java's implied-next everywhere (so `RoleType.Next` reads it and the `IsThreatened` mover-switch preserves it) AND ensuring move *generation* never consumes RNG (only *application* should) so dice games don't desync. Both are core-loop changes with broad blast radius; deferred to a dedicated pass rather than shipped speculatively. Diagnosis recorded here so the future fix is a known quantity. (Boop's `container:0` fix from Update 268 is unaffected and remains committed; units 194/0.)

## Update 270 (2026-06-18) — SHIPPED (dice-guarded): maintain implied next in the end-rule eval (non-dice) — fixes the chaturanga/shatranj false-checkmate
Following Update 269's diagnosis, shipped the faithful fix in a form that carries ZERO regression. The end rule now evaluates with `state.next` stamped to the implied next mover (Step-6's nextMover: moveAgain→same player, SetNextPlayer override honoured, else `(mover % numPlayers)+1`) — but ONLY for games with no hand dice. So `RoleType.Next` / the `(next)` ludeme resolve to the real opponent inside the Checkmate def's `(can Move (do (forEach Piece Next) ifAfterwards:(not ("IsInCheck" K Next))))` instead of player 0, and the false checkmate no longer fires the instant the opponent is in check. Results: Saxun 2 WM→2 OUTCOME_OK; Shatranj at-Tamma / Tepong / Krida Buddhibalasrita each 0→1 OUTCOME_OK of 2 (remaining are long turn-limit trials truncated by MOVE_CAP, not failures); broad stride-12 sample WINNER_MISMATCH 14→10. The DICE GUARD (`handDice().length===0`) leaves dice games (Shatranj al-Mustatila — a dice shatranj) at exactly their prior behaviour: there the end rule's `(forEach Piece Next)` opponent-move generation consumes RNG during GENERATION (a separate latent bug — move generation should never draw RNG, only application should), which would desync the recorded die roll; A/B-confirmed Shatranj al-Mustatila stays 2 OUTCOME_OK with the guard. Only flagged "regression" in the sweep (Shatranj (Algeria)) is a MOVE_CAP truncation artifact — full cap → 2 OUTCOME_OK. Units 194/0. Remaining for a dedicated pass: the RNG-during-generation bug (would let the guard be removed and likely helps dice race families), and L Game's WINNER_MISMATCH (a separate moveAgain/`no Moves Next` turn-structure issue, NOT fixed by this).

## Update 271 (2026-06-18) — `(sites Occupied by:Neutral)` returned ALL occupied sites (move-count explosion)
SitesOccupied's local `roleToIntFunction("Neutral")` fell through to the `default: return -1`. In eval the branch order is `… else if (role === "All" || whoId < 0) … else if (role === "Neutral" || role === "Shared") …`, so a Neutral query (whoId=-1) hit the `whoId < 0` "All" catch-all FIRST and returned every occupied site instead of just the neutral pieces. In Feed the Ducks `(from (sites Occupied by:Neutral))` (the single breadcrumb) became all 25 pieces → a 600-move explosion at the move-generation level. Fix: map RoleType.Neutral → player 0 (`@java RoleType.Neutral.owner()==0`), so whoId 0 skips the `<0` catch-all and routes to the Neutral branch (`owner===0 && what!=0`) → returns just the breadcrumb (24 moves from site 51, was 600). Shared is intentionally NOT mapped (Shared pieces carry owner numPlayers+1, and the Neutral branch checks owner===0): mapping Shared→0 regressed Traffic Lights' `(sites Occupied by:Shared component:"Square")` (all-Shared board), so Shared keeps its prior whoId<0→"All" path; A/B-confirmed Traffic Lights stays 2 OUTCOME_OK. Broad stride-12 sweep: MOVE_MISMATCH 32→31, no real regressions (120 OUTCOME_OK; the lone flagged Shatranj (Algeria) is a MOVE_CAP truncation — full cap → 2 OK); units 194/0. Feed the Ducks itself is NOT yet OUTCOME_OK — a SEPARATE bug remains: after a `(move … (then (attract …)))`, `(last To)` / `Move.toNonDecision()` returns the last attract consequence's `to` instead of the breadcrumb decision's `to`, so the next turn's `(difference (sites Empty) (sites Direction from:(last To)))` excludes the wrong cells. Logged as the next target.

## Update 272 (2026-06-18) — `(attract)` only pulled from HALF the directions (Feed the Ducks)
@java Attract.java iterates `dirnChoice.convertToAbsolute(...)` = EVERY adjacent direction (6 on a hex, 8 on a square) and walks one radial per direction. Our Attract.ts used `radialsForDirection(cellRadials, "Adjacent")`, which returns the distinct AXES (3 on hex, 4 on square) as {ray, opposite} pairs, but walked ONLY `ray` — covering half the directions. So pieces on the opposite-pointing radials were never attracted: Feed the Ducks left a duck stranded at cell 0 instead of pulling it toward the breadcrumb, and by ply 6 cell 0 was wrongly occupied so the recorded `from=51 to=0` breadcrumb move had no match (MOVE_MISMATCH). Fix: walk BOTH `axis.ray` and `axis.opposite` for each axis (the original comment claiming "only ray is used in Java" misread the source). No double-counting (distinctRadialsByName returns 3 hex axes, not 6 rays). Feed the Ducks 2 MOVE_MISMATCH → 1 OUTCOME_OK + 1 WINNER_MISMATCH; `(attract)` is used ONLY by Feed the Ducks so there is no other-game risk; units 194/0. Remaining WM trial: TS falsely fires `("SingleGroupWin" P)` at ply 9 (trials run 45/53 moves) — `IsSingleGroup` over-detects connectivity on the hex board. Logged as the next target (a hex group-flood-fill issue that likely touches other group games).

## Update 273 (2026-06-18) — `(attract)` forced attracted pieces to the mover's ownership (Feed the Ducks FULLY fixed)
Second Attract bug (after Update 272's direction fix): `new ActionAdd({to, what, owner: mover})` forced every re-placed (attracted) piece to the CURRENT player. @java Attract.java's `new ActionAdd(type, to, what, …)` takes NO owner — the owner is derived from the component `what`. In Feed the Ducks every duck is Marker1 (P1) or Marker2 (P2); when P1 moved the breadcrumb, the attract re-placed BOTH players' pulled ducks as P1 (and P2 on P2's turn), scrambling ownership — P1 ended with the wrong piece set, those pieces merged into one group, and `("SingleGroupWin" P1)` = `(= 1 (count Groups if:(= (who at:(to)) P1)))` fired a FALSE win at ply 8 (the trial runs 45 moves). Empirically: TS P1 had 5 sites where the recorded Java board had 12. Fix: derive owner from the component (`ctx.components()[what].owner`), same recipe as the Push owner fix (Update 266). Feed the Ducks is now **2 OUTCOME_OK** (was 2 MOVE_MISMATCH at the start of the session). `(attract)` is used only by Feed the Ducks; units 194/0. Note: `IsSingleGroup`/`CountGroups` were NOT at fault — the earlier "hex group over-detection" hypothesis was wrong; the real cause was the scrambled ownership feeding a correct group count.

## Update 274 (2026-06-18) — DIAGNOSED + DEFERRED: Symple stuck in the Grow-only branch (vars never set)
Symple's play rule: `(if (= (count Moves) (var "MoveThatMustBePlant")) ("Plant") (if (= 0 (count MovesThisTurn)) (or ("Plant") ("Grow")) ("Grow")))`. At ply 25 (mover=2) Java plays a Plant `Add to=165` (165 has 0 own orthogonal neighbours — confirmed: `(sites Around 165 Own Orthogonal)` correctly returns 4 empty neighbours), but TS generates only 22 Grow/PlaceGrey moves (all requiring an own neighbour) and excludes 165. Root cause pinned: in TS `st.getVar("MoveThatMustBePlant") == -1` and `getVar("HasGrown") == -1` — the vars were NEVER SET, even though the board MATCHES Java through ply 24 (so the grey-marker replacement actions did run). Those vars are set only inside `ReplaceAllGrey`'s `(then (if (!= 1 (var "HasGrown")) (and (set Var "HasGrown" 1) (if (= (id P2) (mover)) (and (set Var "MoveThatMustBePlant" (count Moves)) (moveAgain))))))`. So the `(set Var …)` in that then is not persisting in TS even though the surrounding remove/add actions do — and with the vars unset, `(count Moves) == -1` never holds and `(count MovesThisTurn)` reads numTurnSamePlayer=8 (a long Grow moveAgain chain), so TS is permanently in the Grow-only branch and P2 can never Plant. `(set Var)` works in other (passing) var games, so this is specific to Symple's Grow→PlaceGrey→ReplaceAllGrey moveAgain/`(do … next:…)` chain — the var-set rides a nested sub-move whose state delta isn't folding back into the persisted state. NOT the Neutral fix (Symple was MM before it). Next target: trace whether `(set Var)` inside a `(then …)` that itself sits under a `(do … next:…)`/moveAgain chain folds its state into the applied move. Board-match-through-ply-24 + recorded-trial oracle confirm the diagnosis.

## Update 275 (2026-06-18) — Symple deeper: Grow moveAgain-chain never reaches ReplaceAllGrey (set Var path not exercised)
Continued from Update 274. Built and A/B-tested a faithful fix to `Do.eval` Case A (`(do X next:Y)`): the prior X's consequence actions (its `then`, e.g. `(set Var …)`) were applied only to the throwaway temp state, never folded into the persisted compound move — so a prior `(then (set Var …))` was silently dropped. The fix (`_applyPriorWithActions`: collect each prior move's direct actions PLUS its deferred-then `extraActions` and `moveAgain`, prepend the full delta) is correct vs @java Do (prior applied WITH consequences) and A/B-clean (zero regressions / zero new passes across stride-12 (167) AND stride-7 (173) trials). But it did NOT change Symple — its vars stay -1 — because Symple's `Grow` chain `(if (can Move ("PlaceGrey")) (moveAgain) (do ("ReplaceAllGrey") next:("UpdateScore")))` never reaches the `(do (ReplaceAllGrey) …)` branch in TS: TS keeps taking the `(moveAgain)` PlaceGrey branch (`numTurnSamePlayer=8` at ply 25) where Java has already terminated the chain and started a fresh P2 turn that plays a Plant. So the REAL Symple blocker is the Grow `moveAgain` chain not terminating the same way as Java — likely `(can Move ("PlaceGrey"))` staying true too long, where PlaceGrey's `ifAfterwards:(all Groups Orthogonal … if:(> 2 (count Sites in:(intersection (sites Around (sites) NotEmpty Orthogonal) (sites Occupied by:Neutral)))))` (grey-marker count) is sensitive to the Update-271 Neutral fix. The Do-Case-A then-folding fix was REVERTED (closes no game, motivating path unexercised, core-path risk) but is documented here for trivial re-application once a game exercises it with a positive test. Net: Symple remains MM, fully analyzed two layers deep; tree clean, units 194/0.

## Update 276 (2026-06-18) — intArray Union/Intersection single-list form failed to COMPILE (ctor.length drift from `?` params)
A TS optional parameter written as a bare `second?: IntArrayFunction` (no default) STILL counts toward JS `Function.length`, so `Union`/`Intersection`'s ctor reported `.length===2`. The compiler's instantiation drift guard (`if (info.args.length < ctor.length) → "constructor drift"`) then rejected the single-list form `(union {a b c …})` / `(intersection {…})`, which binds exactly ONE arg (the list). Garrisons (`(union {(values Remembered "R-P2") …})`) failed to COMPILE entirely. Fix: give the optional param an explicit `= undefined` default so `.length===1`, matching the truly-required arity; the two-arg form `(union A B)` and the array form both still work (eval is unchanged). Garrisons now compiles (COMPILE_FAIL → it runs; remaining MOVE_MISMATCH is a separate move-gen issue). Difference uses three non-optional `| null` params (always bound to 3) and is unaffected. Broad stride-12 sweep unchanged (120 OUTCOME_OK; only the known Shatranj (Algeria) cap artifact flagged), units 194/0. This is a general compiler-faithfulness fix — any class with a trailing bare-`?` param invoked with fewer args than `.length` would falsely fail; Union/Intersection were the two intArray cases.

## Update 277 (2026-06-19) — `(forEach Group … (then …))` dropped its consequence (frozen-array crash, parallel-agent diagnosis)
ForEachGroup.eval appended the `then` to each move via `mThen.push(...)` on `Move.then`, which is `Object.freeze`d at construction — so the push throws "object is not extensible" the instant a forEach-Group carries a then, and evalDeferredThens silently swallows the error → the consequence is DROPPED. Per-group scoring like `(forEach Group … (then (… (set Score …))))` never ran, leaving states/scores at 0. Fix: bake the consequence in with `applyPostStateThen(thenClause, ctx, m)` (the exact recipe already used by ForEachPiece — ForEachGroup never received it). Refugia → OUTCOME_OK, Mono → OUTCOME_OK, Signum gains a passing trial; broad stride-12 sweep otherwise unchanged (only the known Shatranj (Algeria) cap artifact), units 194/0. Manifold and Brood remain WINNER_MISMATCH — they have a FURTHER scoring issue beyond the dropped then (shape-vector scoring), logged for follow-up. (Diagnosed by a parallel sub-agent.)

## Update 278 (2026-06-19) — Sow leaked its eval-scratch into sibling `(and …)` expressions (Deka; parallel-agent diagnosis)
@java Sow.java walks the track on a TempContext, so its `(to)`/`(from)`/value eval-scratch never leaks back to the caller. Our `Sow.eval` mutated the SHARED `ctx` (`_evalFrom`/`_evalTo`/`_evalValue`) and never restored it. In a relay capture like `(and { (fromTo …) (sow …) (set State at:(to) 1) })`, the nested `(sow …)` left `ctx._evalTo` pointing at the inner sow's LAST hole, so the sibling `(set State at:(to) 1)` marked the WRONG site "covered" — Deka mis-marked holes, the board diverged from ply 0, and a later `(moveAgain)` mis-fired (turn advanced P2→P1 instead of continuing). Fix: snapshot `_evalFrom/_evalTo/_evalValue` once the track is resolved and restore them in a `finally` around the walk, so sibling expressions see the ORIGINAL `(to)`. Deka → 2 OUTCOME_OK; broad stride-12 sweep MOVE_MISMATCH 31→30, 120→121 OUTCOME_OK, units 194/0, no mancala regressions (only the known Shatranj (Algeria) cap artifact). The other two-row mancala MMs (J'erin, Yovodji, Laomuzhu, Tuz, Puhulmutu) have separate/additional issues and remain MM. (Diagnosed by a parallel sub-agent.)

## Update 279 (2026-06-19) — IsConnected with no role flooded the Mover instead of the last-placed piece's owner (Pippinzip; parallel-agent diagnosis)
@java IsConnected: when NO role is given it leaves playerRegion UNDEFINED and floods the group of `who = cs.who(lastTo)` (owner of the last-placed piece), not the mover. TS coalesced `role ?? "Mover"` in the ctor, so a null role floods the MOVER. Pippinzip's "chicken ballot" opening has BOTH players place P2's White piece (`(move Add (piece (id "Disc" P2)) …)`), so the connecting group is owned by P2 even when P1 is the mover; the end `(is Connected All {sides})` (no role) then found no mover-owned group and the game never ended (winner stayed -1). Fix: keep `role` null distinct from explicit "Mover"; when null, derive pid from `cells[(last To)]` (the owner at `_evalTo`), falling back to mover. For normal connection games the mover places their OWN piece so `cells[lastTo] === mover` — identical behaviour; only the place-opponent's-piece case changes. Pippinzip + Zipline variant → 4/4 OUTCOME_OK (were WINNER_MISMATCH); Hex/Havannah/Y/Gonnect unaffected (explicit roles); broad sweep + units 194/0 unchanged. Blast radius: only `(is Connected <dir> {regions})` with no role (Pippinzip variants; experimental Morpharaoh). (Diagnosed by a parallel sub-agent.)

## Update 280 (2026-06-19) — ActionMove in a stacking game PUSHES onto an occupied site (pin), never overwrites (Sam K'i; parallel-agent diagnosis)
@java ActionMoveTopPiece: when `game.isStacking()`, moving onto an occupied site pushes a level — it never overwrites. TS's flat-move path REPLACED the destination when the owner differed (capture-by-replacement), wiping the piece underneath. Fix (action-move.ts, before the same-owner accumulation branch): when `state.stackingGame && movingOwner>0 && who(to)>0 && who(to)!==movingOwner`, `withStackPush(to, movingOwner, movingWhat)` and return — so a checker landing on an enemy PINS it (enemy stays at level 0). Backgammon/Portes-family hits are unaffected: they run an explicit `("HittingCapture")` ActionRemove first, so the destination is empty (who==0) and this branch is skipped — A/B-confirmed Backgammon/Portes/Fallas/Nyout/Svensk/Bashni/Lasca all stay 2 OUTCOME_OK. Net: **Sam K'i** gains a passing trial and a WINNER_MISMATCH clears; broad stride-12 sweep 121→122 OUTCOME_OK / 10→9 WINNER_MISMATCH, units 194/0, zero regressions. The agent's proposed SECONDARY fix (state.ts whatStacks lazy-backfill using baseWhat) was tried and REVERTED — it did not fix its target (Plakoto) and changed nothing measurable, so it was not shipped. **Plakoto itself remains MOVE_MISMATCH**: the pin push is necessary but not sufficient (a further issue in how the pinned single-component pile's identity/levels are tracked); logged for follow-up. (Diagnosed by a parallel sub-agent.)

## Update 281 (2026-06-19) — heterogeneous stacks stored OWNER ids in whatStacks instead of COMPONENT ids (parallel-agent diagnosis ×2)
Two independent sub-agents (race/reach + Ashta-kashte) converged on the same class of bug: `whatStacks` (the per-level COMPONENT-id column) was being populated with OWNER/player numbers, so once upper levels popped, `whats[site]` derived the owner as the component and `(forEach Piece)` could no longer find the piece (forced pass / invisible piece). Two complementary faithful fixes:
1. **ActionAdd (stacking)** — `withStackPush(to, owner)` omitted the `what` arg, so a `(place Stack items:{Pawn1..Pawn4})` heterogeneous start back-filled whatStacks with owner numbers. @java `cs.addItemGeneric(state, to, what, who, …)` pushes BOTH component and owner. Fix: `withStackPush(to, ownerIndex, whatIndex)` (which also sets `whats`), dropping the now-redundant `withWhatAt`. Gyan Chaupar/Es-Sig/Set Dilth'/Siga went from a forced pass (0 moves) to generating the piece's moves (they have a FURTHER ladder/track residual and remain MM).
2. **State.withStackPush back-fill** — the lazy materialization read `this.stacks` (owner ids) to seed the lower `whatStacks` levels. @java the parallel columns seed from component ids. Fix: read `this.whatStacks` (prevWhats), which is empty for a flat-piece site placed via plain ActionMove → falls back to `baseWhat` (the real component); when whatStacks was already materialized it uses the correct existing component. This is the CORRECT version of the backfill change tried+reverted in Update 280 (that one keyed on `!hasWhatStack` and regressed Sam K'i — this one keys per-level on prevWhats and does NOT).
Result: **Ashta-kashte → 1 OUTCOME_OK of 2** (a Pachisi protected-square stack-then-leave now tracked correctly); Backgammon/Portes/Bashni/Lasca/Sam K'i all still OUTCOME_OK (no regression); broad stride-12 sweep 122 OK / 30 MM / 9 WM unchanged, units 194/0. The shipped fixes are correct + exercised + zero-regression; they unblock the heterogeneous-stack identity for the whole Pachisi/stacked-start family even where a further per-game residual (ladders, pinning) keeps some trials MM. (Diagnosed by parallel sub-agents; Shred's count-Steps goRule and Manifold/Brood's region-format fixes were also built but REVERTED — correct yet flipped no trial, deeper residuals remain.)

## Update 282 (2026-06-19) — FromTo ignored `level:` and always popped the stack top (Gyan Chaupar; parallel-agent diagnosis)
With the heterogeneous-stack identity fixed (Update 281), Gyan Chaupar reached a `(move (from … level:(level)) …)` where the piece to move was BURIED: P2 at level 0 under P3 at level 1 on a shared track site. `FromTo` declared `levelFrom` but never evaluated it — it always built a plain `ActionMove`, which pops the stack TOP, so it relocated P3 (the wrong piece). @java FromTo with a `level:` uses ActionMoveLevelFrom → `csFrom.remove(state, from, levelFrom)` (the exact level). Fix: when `levelFrom` resolves to a genuinely NON-TOP level of a true multi-ENTRY stack, build `ActionMoveLevelFrom(from, lv, to)`; otherwise keep the top-popping `ActionMove`. The double guard (`lv < stacks[from].length-1 && stacks[from].length > 1`) is essential — routing top-level or count-backed-pile `level:` moves through ActionMoveLevelFrom regressed Ashta-kashte (its count-pile level: moves rely on plain ActionMove's count handling, which ActionMoveLevelFrom's branch does not replicate). Narrowed to the true-stack buried-piece case only. **Gyan Chaupar → 2 OUTCOME_OK**; Ashta-kashte stays 1 OUTCOME_OK (not regressed); Es-Sig advanced MOVE_MISMATCH→WINNER_MISMATCH (replays fully now, separate winner residual); Backgammon/Bashni/Lasca unaffected; broad stride-12 sweep OUTCOME_OK 122→123 / MOVE_MISMATCH 30→28, units 194/0, zero regressions. (Diagnosed by a parallel sub-agent.)

## Update 283 (2026-06-19) — DIAGNOSED + DEFERRED: Manifold/Brood scoring is a 3-bug chain + an unfound 4th (parallel-agent diagnosis)
A sub-agent traced the all-zero-score tie to a chain: (1) **ForEachValue constructor mis-dispatch** — the range-form check `typeof generatorOrThen.eval === "function"` ALSO matches a `Then` (which has `.eval()`), so `(forEach Value (values Remembered) <gen> (then (forget Value All)))` was routed to the RANGE form, the real generator never ran, and `CountShapesOf` produced nothing; (2) **ForEachValue.eval** never attached its outer `(then …)` (the `(forget Value All)` cleanup); (3) **SitesContext** returned `[]` for the `{sites:[...]}` region ForEachGroup sets, so `(regionSite (sites) 0)` was -1. All three were built (dispatch guard `+ .moves !== function`, applyPostStateThen outer-then, SitesContext `{sites}` reader) and A/B-verified ZERO regression — but Manifold/Brood STILL score 0 (still 2 WINNER_MISMATCH), so a 4th bug downstream (CountShapesOf's inner ForEachGroup / IfShapessMatchDo / RecordMultiplier / the `(set State)`→`(addScore (state at:…))` read) dominates. Per the firm bar (ship only fixes that flip ≥1 trial), the 3-fix bundle was REVERTED (correct + zero-regression but flips nothing without bug 4). All three are documented here for one-shot re-application once bug 4 is found; the ForEachValue dispatch bug in particular is a genuine latent issue for any values-form `(forEach Value … (then …))`. units 194/0, tree clean. (Diagnosed by a parallel sub-agent.)

## Update 284 (2026-06-19) — Manifold & Brood FULLY fixed: 4-bug shape-vector-scoring chain (Bug 4 = bare `(sites)` mis-routed)
Completing Updates 277/283. Manifold/Brood's per-group "shape-vector" scoring tied at 0 due to a FOUR-bug chain, all needed together:
1. **ForEachGroup `(then …)`** dropped its consequence (frozen Move.then push) — already fixed (Update 277).
2. **ForEachValue constructor mis-dispatch**: the range-form guard `typeof generatorOrThen.eval === "function"` also matched a `Then` (Then has `.eval()`), so `(forEach Value (values Remembered) <gen> (then (forget Value All)))` ran the range form and never executed the real generator. Fix: also require `typeof .moves !== "function"` (a Then exposes `.moves()`; a plain generator doesn't).
3. **ForEachValue.eval** never attached its outer `(then …)`. Fix: `applyPostStateThen` per generated move.
4. **Bug 4 — bare `(sites)` resolved to `SitesEquipmentRegion` (returns []) instead of `SitesContext`** (the forEach iteration region). So `(regionSite (sites) index:0)` was -1, every group anchor remembered as -1, `(var "KeySite")` was -1, and `RecordMultiplier`'s `(set State at:(KeySite))` was a no-op on site -1 → all piece states 0 → score 0 (tie). Confirmed by probe: `RegionSite` got `regionClass=SitesEquipmentRegion sites=[]`. Fix: ArgCompiler intercepts bare `(sites)` (no variant) → `new SitesContext()` (gated on the expected RegionFunction type). Plus the SitesContext `{sites:[...]}` reader (so it reads the `{sites}` object ForEachGroup sets).
With all four, **Manifold → 2 OUTCOME_OK and Brood → 2 OUTCOME_OK** (were WINNER_MISMATCH). Mono/Signum also benefit. Broad stride-12 sweep OUTCOME_OK 123→124 / WINNER_MISMATCH 10→9, units 194/0, zero regressions. (Bug 4 found by direct probing after a parallel sub-agent traced bugs 1–3.)

## Update 285 (2026-06-19) — `(where <piece> Neutral)` resolved Neutral to the mover, not player 0 (Es-Sig false early-end; parallel-agent diagnosis)
Where.ts's `wherePlayerFn` had no `Neutral`/`Shared` case, so a `Neutral` role fell through to `return ctx.state.mover`. Es-Sig's `(where "Ghoula" Neutral)` then searched for a MOVER-owned piece, located the wrong site, and fired `GhoulaPhaseDone` ~498 plies early → a false `(byScore)` end at ply 146 with all-zero scores → wrong/tie winner (WINNER_MISMATCH). Fix: `Neutral`/`Shared` → 0 (the @java neutral owner) — the same invariant already shipped for SitesOccupied (Update 271) and Count.roleToInt. Es-Sig's false early-end is eliminated (WINNER_MISMATCH → MOVE_MISMATCH — it now plays on); broad stride-12 sweep otherwise unchanged (the lone flagged Patok is a MOVE_CAP=3000 truncation artifact — full cap → 2 OUTCOME_OK), units 194/0, zero real regressions. **Es-Sig still MOVE_MISMATCH** due to a SECONDARY, higher-blast-radius bug the agent also found: Game.ts's start stack-staging loop (`for li=1` + `if (lv.owner < 1) continue`) skips a Neutral piece at stack level 0 (Es-Sig's Ghoula at the ExternalSite), so once the Sticks pop off, `whats[site]=0` makes `(is Empty (ExternalSite))` prematurely true. Deferred (it changes initial stack materialization for every stacking game — needs careful regression work). (Diagnosed by a parallel sub-agent.)

## Update 286 (2026-06-19) — `(keep …)` graph operator produced a faceless (empty) board (Go with the Floe; parallel-agent diagnosis)
The `(keep …)` graph operator (Keep.ts) built the kept subgraph (vertices + edges inside the polygon) but never called `out.makeFaces()`, so the result had 0 faces. @java Board.java measures the kept graph (`graph.makeFaces()`), regaining its bounded faces (= Cells). Without it, `Board.buildTopology` saw 0 Cell faces, fell back to Vertex play, and the Cell-coordinate start placements (Go with the Floe's `(place "Bear2" {"G2" "G7"})`) found no cells → silently placed nothing → empty board → only a forced Pass. Fix: `out.makeFaces()` before returning the kept graph. Go with the Floe now starts populated and generates real moves (tsMoveCount 1→20; still MOVE_MISMATCH on a further Seal 2-cell-slide residual). A/B-confirmed ConHex (also uses `(keep)`) is unchanged (pre-existing MM) and Suffragetto/Camelot/Patok sweep flags are MOVE_CAP/timing noise (full-cap identical). units 194/0; zero real regressions. Blast radius: only `(keep)`-board games.

## Update 287 (2026-06-19) — DEFERRED diagnoses from this agent batch (need deeper/type-aware work)
- **`(count at:site)` (CountNumber) counts hand-container cells** (J'erin/Yovodji/Tuz sow family): our unified countAt array appends hand cells after board sites, so a hand index returned the hand pile (J'erin's `(from (LastHoleSowed) if:(< 0 (count at:(from))))` emitted a spurious Select vs Java's Pass). BUT a pure index bound is WRONG: `board.numSites` is the cell count on Vertex-play boards (Morris) — bounding by it broke ~25 games; `sitesFrom[1]` also broke Morris/Tapatan/Dara, which legitimately COUNT their hand via bare `(count at:(handSite))`. The faithful fix is TYPE-AWARE (Java `cs.count(site, defaultType)` returns 0 when the site's container type ≠ the default), requiring per-site container-type resolution. Deferred. (Tuz flipped 1 trial under the naive fix but it regressed 25 — reverted.)
- **Garrisons**: `(count Steps … (step … (to if:…)) …)` ignores the step goRule (same CountSteps gap as Shred) → the skirting BFS reaches all empty sites and over-generates; also the harness placement-matcher masks the early divergence. Needs the CountSteps stepMove/goRule wiring (built for Shred, found inert there) AND care around the matcher. Deferred.
- **Hunt cluster (Shui Yen / Juroku Musashi / Shi Liu Kan)**: `SitesOuter` uses a rectangular-grid fallback (wrong for merged/concentric boards — should use `traj.perimeterSites()`), and `ActionAdd` OVERWRITES an occupied site during setup instead of (Java) incrementing count and leaving the existing piece — destroying P2's piece when P1's `(place (sites Outer))` overlaps it. The ActionAdd change is high-blast-radius (every placement) — needs isolated regression work. Deferred.

## Update 288 (2026-06-19) — `(layer of:…)` always returned -1 on Vertex/Edge boards (Shibumi family; parallel-agent diagnosis)
Layer.eval resolved the site type via `context.game.board?.().defaultSite()`, but the board lives at `game.equipment.board` (Game has no `board()` method), so it silently fell back to `"Cell"`. On a `use:Vertex` Shibumi board this queried the 14 Cell elements with vertex indices 0–29 → out-of-range → `(layer of:…)` returned -1. The line-length end rules `(is Line (- (count Rows) (layer of:(last To))) SameLayer …)` then required impossible 5-length lines and never fired → winner stayed -1. Fix: use `context.board().defaultSite()` (the Context's own facade, which resolves the real play-site type). **Spava → 2 OUTCOME_OK**; Spline/Pylos/Spree/Madrod also benefit; broad stride-12 sweep OUTCOME_OK 124→126 / WINNER_MISMATCH 9→7, units 194/0, zero regressions (Cell boards unchanged — fallback was already "Cell"). Blast radius: every `(layer of:)` game on a Vertex/Edge board (Shibumi family). (Diagnosed by a parallel sub-agent.)

## Update 289 (2026-06-19) — `(keep (poly …))` inflate used radial expansion instead of Java's edge-normal deflate (Go with the Floe FULLY fixed; parallel-agent diagnosis)
Following Update 286 (makeFaces). The `inflate` helper in Keep.ts moved each polygon vertex radially OUTWARD from the centroid (expanding), but @java `main.math.Polygon.inflate(amount)` adjusts each vertex along the AVERAGE of its two adjacent edge directions (`Vector(ptB,ptA)`+`Vector(ptB,ptC)` for a CCW poly, normalized, ×0.5) — which for a CCW polygon points INWARD, i.e. it DEFLATES. Go with the Floe's diamond `(keep (poly {{-3 4}{4 -3}{11 4}{4 11}}))` therefore kept 60 cells (L1 radius grew 7.0→7.14) instead of Java's 52 (radius shrank to ~6.93), giving a different board topology and site numbering — every recorded move referenced sites that didn't exist in TS. Fix: faithful port of Polygon.inflate (per-vertex edge-normal average, with MathRoutines.clockwise(EPSILON=1e-7) selecting the Vector orientation; `Vector(a,b)=b−a`). Go with the Floe → **2 OUTCOME_OK** (with the makeFaces fix). ConHex unchanged (pre-existing MM, different `(keep)` polygon); broad sweep 126/28/7 unchanged, units 194/0, zero regressions. Blast radius: `(keep (poly …))` board games.

## Update 290 (2026-06-19) — TOP DEFERRED: race/escape internal-loop track cluster (20 games; parallel-agent diagnosis)
~20 board/race/escape games (Pachisi/Tables with internal-loop tracks: Kints, Bargese, Chaupar, Pachesi, Sokkattan, …) MOVE_MISMATCH because the on-track index structure is never built. A site that appears TWICE on a looped track (e.g. Kints site 1 at ring index 5=arc-entry and 43=near-End) is resolved at the WRONG index → wrong destination (TS computes a normal move where Java bears off). Complete 3-part fix (diagnosed, NOT yet applied — needs a track-representation BRIDGE): (1) add `Game.hasInternalLoopInTrack()` = `getTracks().some(t => t.hasInternalLoop())`; (2) initialize `state.onTrackIndices`/`trackLocToIndex` in Game.start via `buildTrackLocToIndex`+`buildInitialOnTrackIndices` (the helpers in on-track-indices.ts exist and the per-move maintenance in action-move/remove is already wired but dead because the initial value is undefined); (3) rewrite TrackSiteMove.eval's internal-loop lookup (lines ~247-259) from Java-style API (`state().onTrackIndices().locToIndex(...).getQuick()`) to the TS shape (`state.onTrackIndices: number[][][]`, `state.trackLocToIndex: Map<number,number[]>[]`). BRIDGE NEEDED: the Track CLASS from `board.getTracks()` exposes `internalLoop`/`hasInternalLoop()`/`_elems` but NOT `.sites`/`.locToIndex` that buildTrackLocToIndex (MancalaTrack interface) reads — must adapt (derive sites from `_elems`, build locToIndex). NATURALLY LOW BLAST RADIUS: buildTrackLocToIndex returns undefined unless a track has internalLoop, so non-loop track games (Backgammon/mancala) are untouched. Highest-value single remaining cluster — next focused pass.

## Update 291 (2026-06-19) — internal-loop cluster: 3-part fix BUILT & infrastructure VERIFIED, but doesn't flip yet (reverted)
Implemented the Update-290 3-part fix and verified the infrastructure works: `Game.hasInternalLoopInTrack()` returns true for Kints; Game.start now builds `state.onTrackIndices`/`trackLocToIndex` via a Track→MancalaTrack adapter (extract sites from `elems()`, derive locToIndex) — confirmed Kints track0 `locToIndex.get(1) = [5,43]` (the two occurrences) and onTrackIndices allocated; TrackSiteMove rewritten to the TS state shape. Zero regression (Backgammon/Kalah/Morris stay OUTCOME_OK — the init is gated on internalLoop so non-loop games are untouched). BUT Kints/Bargese/Pachisi/Sokkattan still MOVE_MISMATCH. Probing showed the divergent path is NOT the one diagnosed: TrackSiteMove.eval is invoked only **2× total** in the whole Kints replay (cap 5000), BOTH with `what=0` — so the `(!islooped && hasInternalLoopInTrack)` branch takes the `what===0` linear-scan path (first occurrence = wrong index 5), and the onTrackIndices `what!=0` lookup never runs. Likely causes: the harness's SCRIPTED_DICE move-matching shortcuts full track move-generation, and/or `cs.what(currentLoc, defaultSite)` returns 0 at the from-site for this board (so the piece is never identified), and/or the per-move maintenance (`maintainOnTrackIndicesForMove`) isn't advancing the count along the loop. The 3-part change was REVERTED (flips no trial — discipline) but the working infra is documented here for one-shot re-application; the focused next step is to root-cause the `what=0`/hot-path (re-trace the LIVE Kints bear-off move generation, not the static diagnosis).

## Update 292 (2026-06-19) — `(sites Outer)` fabricated a W×H grid border instead of the true graph perimeter (hunt cluster; parallel-agent diagnosis)
SitesOuter.eval fell back to a rectangular `row==0||row==H-1||col==0||col==W-1` grid border, which is WRONG for merged/concentric/irregular boards. The hunt games place P1 on `(sites Outer)`: on Shui Yen/Juroku Musashi/Shi Liu Kan's merged cross-board the fake border put pieces on non-perimeter sites. Fix: use the trajectories' `perimeterSites()` (the true graph boundary) before the rectangular fallback — for a plain rectangle it equals the border row/col, so square games are unchanged. **Juroku Musashi → 2 OUTCOME_OK, Shi Liu Kan Tsiang Kun → 2 OUTCOME_OK**; broad stride-12 sweep OUTCOME_OK 126→127 / MOVE_MISMATCH 28→27; A/B-confirmed Surakarta pre-existing MM (not regressed), Fanorona/Quixo unaffected; units 194/0. Shui Yen Ho-Shang still MM — it ALSO needs the (deferred, high-blast-radius) ActionAdd overwrite→count fix (P1's `(sites Outer)` placement overlaps P2's monk and currently overwrites it). (Diagnosed by a parallel sub-agent.)

## Update 293 (2026-06-19) — `(forEach Piece Neutral)` resolved Neutral to the mover, not player 0 (Un)
ForEachPiece's `roleToIntFunction` handled Shared (→numPlayers+1) but had no Neutral case, falling through to the mover. Un's Main phase `(play (forEach Piece Neutral))` (both players slide the shared NEUTRAL Markers) therefore iterated the mover's nonexistent pieces and generated no slides. Fix: `Neutral → 0` (the @java neutral owner) — the 4th site of the recurring Neutral→0 invariant (after SitesOccupied/Count/Where). **Un → 2 OUTCOME_OK**; broad stride-12 sweep OUTCOME_OK 127→128 / MOVE_MISMATCH 27→26; Pasang/Tablut/International Draughts unaffected; units 194/0; zero regressions.

## Update 294 (2026-06-19) — FromTo didn't lay a LARGE tile's footprint on a board→board move (Pentomino + L Game; parallel-agent diagnosis)
`FromTo.eval` emitted a bare `ActionMove({from,to})` for every (from,to), transferring only the ANCHOR cell — it lacked the large-piece (tile-with-walks) handling that `Add.ts` already has. A pentomino move thus filled 1 cell instead of 5: `(sites Empty)` stayed too large, `(no Moves Next)` never fired, and the game never ended (winner -1). Fix: when `from` holds a piece with `walks`, enumerate (anchor `to`, rotation `st`) like Add.ts — `Add.locsLargePiece` (made public + reused), require every footprint cell empty, emit `ActionMove({from,to,state:st,footprint:locs})` (action-move.ts already lays the footprint). Gated on `walks`, so non-tile FromTo games are untouched. **Pentomino → 2 OUTCOME_OK; L Game → 1 OUTCOME_OK of 2** (was WINNER_MISMATCH); broad stride-12 sweep OUTCOME_OK 128→129 / WINNER_MISMATCH 7→6; Cram/Domineering (Add-path tiles) unaffected; units 194/0; zero regressions.

## Update 295 (2026-06-19) — internal-loop cluster: TrackSiteMove is NOT the live path (3rd revert; needs fresh trace)
Re-applied the 3-part fix and instrumented: TrackSiteMove.eval is reached only with `currentLoc=-1` (returns OFF before the what-lookup) — it is NEVER invoked for Kints' ply-20 bear-off (from=1). So the `to=10` (wrong) vs End (Java bear-off) destination is computed by a DIFFERENT code path, NOT TrackSiteMove. Both sub-agents mis-identified TrackSiteMove as the culprit (it's the natural `(trackSite Move …)` class, but Kints' `("NextSiteOnTrack" ("ThrowValue"))` must expand to something else, or the live move-gen routes elsewhere). The onTrackIndices infrastructure (init + adapter) is correct and verified (Kints locToIndex.get(1)=[5,43]) but irrelevant until the REAL destination path is found. Reverted all 3 parts again. NEXT: trace what `NextSiteOnTrack`/`ThrowValue` actually compile to and which TS function returns 10 for (from=1, steps=2) on the looped track — that is the true fix site.

## Update 296 (2026-06-19) — DEFERRED: Shibumi `(meta (gravity))` is a no-op stub (Spline+/Spought/Shibumi family; parallel-agent diagnosis)
`Gravity.eval()` (src/ludemes/game/rules/meta/Gravity.ts) is a no-op — unsupported balls never fall. @java Game.computeLegalMoves applies Gravity to each move: simulate it, then while any occupied vertex has an empty `Downward` neighbour, append `ActionMove(site→below)` and repeat. Spline+ ply 31: Java drops P1 from site 25→28 after P2 vacates 28; TS leaves 25 occupied, so `IsFlat(25)` is false and the recorded `from=12 to=25` move (ply 33) is rejected. Fix options: (a) embed gravity drops as `then` actions during move-gen (faithful, matches the recorded action list), or (b) apply gravity post-hoc in Game.apply (simpler; board ends correct so subsequent move-gen is right; the harness matches by decision from/to so the unembedded drops don't break matching). The Downward trajectory infra works (`traj.steps(25,"Downward")=[20,22,28,29]`), `IsFlat` is correct. Blast radius: only `(meta (gravity))` games (Shibumi family — Spline+, Spought, etc.). Multi-step state-simulation; deferred to a focused pass.
