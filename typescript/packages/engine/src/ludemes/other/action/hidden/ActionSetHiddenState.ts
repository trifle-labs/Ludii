...
/**
 * Sets the state hidden information to a graph element type at a specific level
 * for a specific player.
 *
 * Faithful 1:1 transliteration of other.action.hidden.ActionSetHiddenState.
 *
...
export class ActionSetHiddenState extends BaseAction {
  ...
  apply(context: LudiiContext, _store: boolean): this {
    ...
    if (!this.alreadyApplied) {
      ...
      this.previousValue = cs.isHiddenState(this.who, this.toSite, this.levelField, this.typeField);
      this.previousType = this.typeField;
      this.alreadyApplied = true;
    }
    context.containerState(context.containerId()[this.toSite])
      .setHiddenState(context.state(), this.who, this.toSite, this.levelField, this.typeField, this.valueField);
    return this;
  }

  undo(context: LudiiContext, _discard: boolean): this {
    context.containerState(context.containerId()[this.toSite])
      .setHiddenState(context.state(), this.who, this.toSite, this.levelField, this.previousType, this.previousValue);
    return this;
  }

  toTrialFormat(context: LudiiContext | null): string {
    let sb = "[SetHiddenState:";
  ...
  getDescription(): string { return "SetHiddenState"; }
  ...
  override toTurnFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return this.toSite + (this.levelField !== UNDEFINED ? "/" + this.levelField : "") +
      "P" + this.who + (this.valueField ? "=HiddenState" : "!=HiddenState");
  }

  override toMoveFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return "(HiddenState at " + this.toSite +
  ...
  override actionType(): ActionType { return "SetHiddenState"; }
}
