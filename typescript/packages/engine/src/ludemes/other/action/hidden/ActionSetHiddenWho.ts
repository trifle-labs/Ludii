...
/**
 * Sets the who hidden information to a graph element type at a specific level
 * for a specific player.
 *
 * Faithful 1:1 transliteration of other.action.hidden.ActionSetHiddenWho.
 *
...
export class ActionSetHiddenWho extends BaseAction {
  ...
  apply(context: LudiiContext, _store: boolean): this {
    ...
    if (!this.alreadyApplied) {
      ...
      this.previousValue = cs.isHiddenWho(this.who, this.toSite, this.levelField, this.typeField);
      this.previousType = this.typeField;
      this.alreadyApplied = true;
    }
    context.containerState(context.containerId()[this.toSite])
      .setHiddenWho(context.state(), this.who, this.toSite, this.levelField, this.typeField, this.valueField);
    return this;
  }

  undo(context: LudiiContext, _discard: boolean): this {
    context.containerState(context.containerId()[this.toSite])
      .setHiddenWho(context.state(), this.who, this.toSite, this.levelField, this.previousType, this.previousValue);
    return this;
  }
  ...
  getDescription(): string { return "SetHiddenWho"; }
  ...
  override toTurnFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return this.toSite + (this.levelField !== UNDEFINED ? "/" + this.levelField : "") +
      "P" + this.who + (this.valueField ? "=HiddenWho" : "!=HiddenWho");
  }

  override toMoveFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return "(HiddenWho at " + this.toSite +
  ...
  override actionType(): ActionType { return "SetHiddenWho"; }
}
