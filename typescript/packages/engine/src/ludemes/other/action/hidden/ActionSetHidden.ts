 ...
 * specific player.
 *
 * Faithful 1:1 transliteration of other.action.hidden.ActionSetHidden.
 *
...
export class ActionSetHidden extends BaseAction {
  // -------------------------------------------------------------------------
  ...
  apply(context: LudiiContext, _store: boolean): this {
    ...
    if (!this.alreadyApplied) {
      ...
      this.previousValue = cs.isHidden(this.who, this.toSite, this.levelField, this.typeField);
      this.previousType = this.typeField;
      this.alreadyApplied = true;
    }
    context.containerState(context.containerId()[this.toSite])
      .setHidden(context.state(), this.who, this.toSite, this.levelField, this.typeField, this.valueField);
    return this;
  }

  undo(context: LudiiContext, _discard: boolean): this {
    context.containerState(context.containerId()[this.toSite])
      .setHidden(context.state(), this.who, this.toSite, this.levelField, this.previousType, this.previousValue);
    return this;
  }
  ...
  getDescription(): string { return "SetHidden"; }
  ...
  override actionType(): ActionType { return "SetHidden"; }
}
