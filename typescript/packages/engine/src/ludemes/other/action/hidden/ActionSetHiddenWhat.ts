 ...
 *
 * Faithful 1:1 transliteration of other.action.hidden.ActionSetHiddenWhat.
 *
...
export class ActionSetHiddenWhat extends BaseAction {
  ...
  apply(context: LudiiContext, _store: boolean): this {
    ...
    if (!this.alreadyApplied) {
      ...
      this.previousValue = cs.isHiddenWhat(this.who, this.toSite, this.levelField, this.typeField);
      this.previousType = this.typeField;
      this.alreadyApplied = true;
    }
    context.containerState(context.containerId()[this.toSite])
      .setHiddenWhat(context.state(), this.who, this.toSite, this.levelField, this.typeField, this.valueField);
    return this;
  }

  undo(context: LudiiContext, _discard: boolean): this {
    context.containerState(context.containerId()[this.toSite])
      .setHiddenWhat(context.state(), this.who, this.toSite, this.levelField, this.previousType, this.previousValue);
    return this;
  }

  toTrialFormat(context: LudiiContext | null): string {
    let sb = "[SetHiddenWhat:";
  ...
  getDescription(): string { return "SetHiddenWhat"; }
  ...
  override toTurnFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return this.toSite + (this.levelField !== UNDEFINED ? "/" + this.levelField : "") +
      "P" + this.who + (this.valueField ? "=HiddenWhat" : "!=HiddenWhat");
  }

  override toMoveFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return "(HiddenWhat at " + this.toSite +
  ...
  override actionType(): ActionType { return "SetHiddenWhat"; }
}
