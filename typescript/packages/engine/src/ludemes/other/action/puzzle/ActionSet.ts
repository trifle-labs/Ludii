 ...
 *
 * Faithful 1:1 transliteration of other.action.puzzle.ActionSet.
 *
...
export class ActionSet extends BaseAction {
  ...
  apply(context: LudiiContext, _store: boolean): Action {
    ...
    if (!this.alreadyApplied) {
      const maxVal: number = context.game().variables() ?? 10;
      this.previousValues = new Array(maxVal + 1).fill(false);
      for (let v = 1; v <= maxVal; v++) {
        this.previousValues[v] = cs.isPossible(this.varIndex, v, this.typeField);
      }
      this.alreadyApplied = true;
    }

    cs.setResolved(context.state(), this.varIndex, true, this.typeField);
    // Mark only `valueField` as possible; exclude all others
    const maxVal: number = this.previousValues.length - 1;
    for (let v = 1; v <= maxVal; v++) {
      cs.setPossible(context.state(), this.varIndex, v, (v === this.valueField), this.typeField);
    }
    return this;
  }
  ...
  undo(context: LudiiContext, _discard: boolean): Action {
    ...
    const maxVal: number = this.previousValues.length - 1;
    for (let v = 1; v <= maxVal; v++) {
      cs.setPossible(context.state(), this.varIndex, v, this.previousValues[v], this.typeField);
    }
    cs.setResolved(context.state(), this.varIndex, false, this.typeField);
    return this;
  }
  ...
  getDescription(): string { return "SetValuePuzzle"; }
  ...
  override toMoveFormat(_ctx: LudiiContext | null, _useCoords: boolean): string {
    return "(Set var=" + this.varIndex + " value=" + this.valueField + ")";
  }
  ...
  override actionType(): ActionType { return "SetValuePuzzle"; }
}
