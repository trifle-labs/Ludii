...
export class Remove implements MovesFunction {
  ...
  public eval(ctx: Context): Move[] {
    ...
    for (const loc of locs) {
      ...
      const actionRemove = new ActionRemove({ to: loc, ...(this.type ? { type: this.type as never } : {}), ...(applyNow ? {} : { endOfTurn: true }) });
      actions.push(actionRemove);

      // @java Remove.java:144-149 — additional removes for count > 1
      let remaining = count - 1;
      while (remaining > 0) {
        actions.push(new ActionRemove({ to: loc, ...(this.type ? { type: this.type as never } : {}), ...(applyNow ? {} : { endOfTurn: true }) }));
        remaining--;
      }
