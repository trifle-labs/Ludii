...
export function boardSides(ctx: Context, dirName: string): number[] {
  ...
  // @java MeasureGraph.measureSides + findSides + sideFromRun — sides are
  // computed on the VERTEX perimeter ring: corners (MeasureGraph corner
  // scoring) split the ring into runs, each run is classified by the
  // discrete direction (16 buckets) of its centroid from the graph centroid
  // (exact cardinals N/E/S/W only at buckets 4/0/8/12 — a point-up hexhex
  // like Annuvin's (rotate 30 (hex 4)) gets its N side ONLY from the short
  // apex run at the top corner), and every vertex of the INCLUSIVE run is on
  // that side. Faces then inherit every side bit of any of their vertices
  // (@java measureSides:781-788). An earlier CELL-ring shortcut classified
  // runs on cell centroids, which merges the apex runs away (Annuvin lost
  // Side N and started a piece short per side); the vertex-run path below
  // reproduces Java for both HexDame (SE = {0,5,11,18,26}) and Annuvin
  // (N = {36}).
