/**
 * @java Core/src/game/functions/graph/operators/Clip.java
 * Returns the result of clipping a graph to a specified polygon shape.
 * Vertices INSIDE the polygon are REMOVED (opposite of Keep).
 */
...
/**
 * Clip operator: remove all vertices inside the polygon and their edges.
 * @java game/functions/graph/operators/Clip.java
 */
export class Clip extends BaseGraphFunction {
  private readonly graphFn: GraphFunction;
  private readonly polygon: ReadonlyArray<Pt>;

  /** @java Clip(GraphFunction graphFn, Poly poly) */
  ...
  /** @java Clip.eval(Context, SiteType) */
  public override eval(siteType: string): Graph {
    ...
    // @java Clip.eval — mark vertices INSIDE polygon for removal
    const removeSet = new Set<number>();
    for (const v of source.vertices)
      if (polyContains(poly, v.x, v.y)) removeSet.add(v.id);

    // Build new graph keeping vertices NOT in the polygon
    const out = new Graph();
