// @java Core/src/game/functions/ints/count/site/CountNeighbours.java
...
export function compileCountNeighbours(node: LudList, env: CompileEnv): IntFn {
  const siteFn = compileSiteSource(node, env);
  return {
    // Java CountNeighbours.eval reads the first region/at site, then counts the
    // graph element's neighbours list (CountNeighbours.java:62-90).
    eval: (ctx) => countFor(ctx, siteFn(ctx), "Adjacent"),
  };
}
...
register("int", "count:Neighbours", compileCountNeighbours as any);
