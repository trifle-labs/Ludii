// @java Core/src/game/functions/ints/count/site/CountDiagonal.java
...
export function compileCountDiagonal(node: LudList, env: CompileEnv): IntFn {
  const siteFn = compileSiteSource(node, env);
  return {
    // Java CountDiagonal.eval reads the first region/at site, then counts the
    // graph element's diagonal list; edges return 0 (CountDiagonal.java:62-89).
    eval: (ctx) => countFor(ctx, siteFn(ctx), "Diagonal"),
  };
}
...
register("int", "count:Diagonal", compileCountDiagonal as any);
