// @java Core/src/game/functions/ints/count/site/CountAdjacent.java
...
export function compileCountAdjacent(node: LudList, env: CompileEnv): IntFn {
  const siteFn = compileSiteSource(node, env);
  return {
    // Java CountAdjacent.eval reads the first region/at site, then counts the
    // graph element's adjacent list (CountAdjacent.java:63-91).
...
register("int", "count:Adjacent", compileCountAdjacent as any);
