...
export function compileGe(node: LudList, env: CompileEnv): BoolFn {
  ...
  // Non-puzzle Java branch compares IntFunction evals directly (Ge.java:58-65).
  // TODO: deduction puzzles need ContainerDeductionPuzzleState (Ge.java:67-77).
  return { eval: (ctx) => valueA.eval(ctx) >= valueB.eval(ctx) };
}

register("bool", ">=", compileGe as any);
