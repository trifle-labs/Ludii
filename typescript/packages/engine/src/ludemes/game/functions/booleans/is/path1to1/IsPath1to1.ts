...
export class IsPath1to1 implements BooleanFunction {
...
registerBool1to1("is:path", (node: LudNode, env: Compile1to1Env): BooleanFunction => {
  const { positional, named } = parseArgs1to1((node as LudList).items);
  // Java: IsPath(SiteType type, IntFunction from, Player who/RoleType role,
  //              RangeFunction range, BooleanFunction closed)
  // LUD syntax: (is Path <type> [from:<site>] <who/role> <range> [closed:<bool>])
  // positional[0] = "Path"
  // positional[1] = SiteType ident
  // positional[2] = who/role ident
  // positional[3] = range (IntFunction for min, or number)
  // named: from, closed
  ...
  if (fromNode) {
    try { fromFn = compileInt1to1(fromNode); }
    catch { fromFn = { eval: (c: Context & EvalScratch) => c._evalTo }; }
  } else {
    fromFn = { eval: (c: Context & EvalScratch) => c._evalTo };
  }

  // who/role: positional or use mover default
  const whoNode = positional[idx];
  idx++;
  const whoFn = makeWhoFn(whoNode);

  // range: next two positionals or a single number
  // Java RangeFunction: (range min max) or single IntFunction
  let minLenFn: IntFunction = { eval: () => 1 };
  let maxLenFn: IntFunction = { eval: () => INFINITY };
  const rangeNode = positional[idx];
  idx++;
  if (rangeNode) {
    const isLudList = (n: LudNode): n is LudList => 'items' in n;
    if (isLudList(rangeNode)) {
      // (range min max) form
      const ri = rangeNode.items;
      if (ri.length >= 2) {
        try { minLenFn = compileInt1to1(ri[1]); } catch { /* keep */ }
      }
      if (ri.length >= 3) {
        try { maxLenFn = compileInt1to1(ri[2]); } catch { maxLenFn = minLenFn; }
      } else {
        maxLenFn = minLenFn;
      }
    } else if (isNumber(rangeNode)) {
      const v = rangeNode.value;
      minLenFn = { eval: () => v };
      maxLenFn = { eval: () => v };
    } else {
      try {
        const compiled = compileInt1to1(rangeNode);
        minLenFn = compiled;
        maxLenFn = compiled;
      } catch { /* keep defaults */ }
    }
  ...
  let closedFn: BooleanFunction = { eval: () => false };
  if (closedNode) {
    try { closedFn = compileBool1to1(closedNode, env.numPlayers); } catch { /* keep */ }
  }

  return new IsPath1to1(indexType, fromFn, whoFn, minLenFn, maxLenFn, closedFn);
});
