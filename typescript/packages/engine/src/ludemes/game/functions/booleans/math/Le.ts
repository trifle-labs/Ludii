...
export function compileLe(node: LudList, env: CompileEnv): BoolFn {
  ...
  // Non-puzzle Java branch compares IntFunction evals directly (Le.java:58-65).
  // TODO: deduction puzzles need ContainerDeductionPuzzleState (Le.java:67-76).
  return { eval: (ctx) => valueA.eval(ctx) <= valueB.eval(ctx) };
}

register("bool", "<=", compileLe as any);
