...
registerBool1to1("is:pattern", (node: LudNode, _env: Compile1to1Env): BooleanFunction => {
  const { positional, named } = parseArgs1to1((node as LudList).items);
  // Java: IsPattern(StepType[] walk, SiteType type, IntFunction from, IntFunction what/whats[])
  // LUD syntax: (is Pattern <walk> [<type>] [from:<site>] [what:<int>] [whats:<int[]>])
  // positional[0] = "Pattern"
  // positional[1] = walk (LudList of F/R/L)
  // positional[2..] = optional type, etc.

  // Find the walk node (a list containing F/R/L idents)
  let walk: StepType[] = [];
  let idx = 1;
  const p1 = positional[idx];
  if (p1 && isList(p1)) {
    walk = parseWalk(p1);
    idx++;
  }
  ...
  // Skip optional SiteType ident
  {
    ...
    if (p && isIdent(p)) {
      ...
      if (n === "edge" || n === "cell" || n === "vertex") idx++;
    }
  }

  // from: named or default = LastTo
  ...
  if (whatsNode && isList(whatsNode)) {
    whatsFn = [];
    for (const item of whatsNode.items.slice(1)) {
