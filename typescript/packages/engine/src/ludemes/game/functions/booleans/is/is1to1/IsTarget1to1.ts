...
export class IsTarget1to1 implements BooleanFunction {
  ...
  public constructor(configuration: readonly number[], specificSites: readonly number[] | null) {
    this.configuration = configuration;
    this.specificSites = specificSites;
  }
...
registerBool1to1("is:target", (node: LudNode, _env: Compile1to1Env): BooleanFunction => {
  ...
  // optional: named "at" = curly list of site integers
  ...
  // Parse specificSites from named "at" or positional[2]
  let specificSites: number[] | null = null;
  const atNode = named.get("at") ?? positional[2];
  if (atNode && isList(atNode) && atNode.delimiter === "curly") {
    specificSites = [];
  ...
  return new IsTarget1to1(configuration, specificSites);
});
