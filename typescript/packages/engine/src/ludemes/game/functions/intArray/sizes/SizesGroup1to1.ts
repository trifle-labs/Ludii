...
registerIntArray1to1("sizes", (node: LudNode, env: Compile1to1Env): IntArrayFunction => {
  ...
  if (typeName !== "group") {
    // Only Group is implemented in Java via Sizes.construct
    return { eval: (_ctx: Context) => [] };
  }

  // Parse direction (second positional arg if ident and not a role/player)
  let dirName = "Adjacent";
  const dirNode = positional[1];
  if (dirNode && isIdent(dirNode)) {
    const dn = dirNode.name;
    // Skip role-like idents (P1/P2/Mover/Next/All/Ally/Enemy/Friend/NonMover)
    if (!/^(P\d+|Mover|Next|All|Ally|Enemy|Friend|NonMover)$/i.test(dn)) {
      dirName = dn;
    }
  }

  // Determine who-fn: role positional, of: named, or If: named
  ...
  if (ofNode) {
    whoFn = compileInt1to1(ofNode);
  } else if (ifNode) {
    // condition-only: allPieces scope
    whoFn = { eval: (_ctx: Context) => 0 };
    condFn = compileBool1to1(ifNode, env.numPlayers);
    allPieces = true;
  } else {
    // Find role positional (skip first = "Group", skip second = direction if present)
    let roleNode: LudNode | undefined;
    for (let i = 1; i < positional.length; i++) {
      const n = positional[i];
      if (n && isIdent(n) && n !== dirNode) {
        const nm = n.name.toLowerCase();
        if (/^(p\d+|mover|next|all|ally|enemy|friend|nonmover)$/.test(nm)) {
          roleNode = n;
          break;
        }
      }
    ...
    if (roleNode && isIdent(roleNode)) {
      ...
      if (roleName === "all" || roleName === "shared") {
        allPieces = true;
        whoFn = { eval: (_ctx: Context) => 0 };
      ...
      // Default: All (SizesGroup.java:91-98)
      allPieces = true;
      whoFn = { eval: (_ctx: Context) => 0 };
    }
  }

  const minFn: IntFunction = minNode
    ? compileInt1to1(minNode)
    : { eval: (_ctx: Context) => 0 };

  return new SizesGroup1to1(whoFn, minFn, condFn, allPieces, dirName);
});
