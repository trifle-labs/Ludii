// @java Core/src/metadata/ai/heuristics/terms/CornerProximity.java
 ...
 */
export class CornerProximity extends HeuristicTerm {
  ...
  override copy(): CornerProximity {
    const c = new CornerProximity(this.transformation, this.weight, null);
  ...
  override merge(term: HeuristicTerm): void {
    const castTerm = term as CornerProximity;
    for (let i = 0; i < this.pieceWeightNames.length; i++) {
  ...
  override description(): string {
    return "Proximity of pieces to the corners of the board.";
  }

  override toEnglishString(_context: unknown, _playerIndex: number): string {
    const dir = this.weight > 0 ? "move towards" : "move away from";
    return `You should try to ${dir} the corners of the board (weight: ${this.weight})\n`;
  }
