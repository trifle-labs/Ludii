// @java AI/src/decision_trees/classifiers/ExperienceImbalancedBinaryClassificationTree2Learner.java
 ...
 *
 * @java decision_trees/classifiers/ExperienceImbalancedBinaryClassificationTree2Learner.java
 ...
 * where the "True" branch must always directly end in a leaf node.
 *
 * @java decision_trees.classifiers.ExperienceImbalancedBinaryClassificationTree2Learner
 */
export class ExperienceImbalancedBinaryClassificationTree2Learner {

  //-------------------------------------------------------------------------

  /**
   * Builds an exact logit tree node for given feature set and experience buffer
   * @java ExperienceImbalancedBinaryClassificationTree2Learner.buildTree(BaseFeatureSet, LinearFunction, ExperienceBuffer, int, int)
   */
  public static buildTree(
    ...
    return ExperienceImbalancedBinaryClassificationTree2Learner.buildNode(
      featureSet,
      allFeatureVectors,
      allTargetLabels,
      new Set<number>(),
  ...
  /**
   * @java ExperienceImbalancedBinaryClassificationTree2Learner.buildNode(BaseFeatureSet, List<FeatureVector>, TFloatArrayList, BitSet, BitSet, int, int, int, int)
   */
  private static buildNode(
    ...
    const meanProbsIfFalseAspatial = new Array<number>(numAspatialFeatures).fill(0);
    const meanProbsIfTrueAspatial = new Array<number>(numAspatialFeatures).fill(0);
    const meanProbsIfFalseSpatial = new Array<number>(numSpatialFeatures).fill(0);
    const meanProbsIfTrueSpatial = new Array<number>(numSpatialFeatures).fill(0);

    for (let i = 0; i < numAspatialFeatures; ++i) {
      if (numFalseAspatial[i]! > 0) meanProbsIfFalseAspatial[i] = sumProbsIfFalseAspatial[i]! / numFalseAspatial[i]!;
    ...
    for (let i = 0; i < numSpatialFeatures; ++i) {
      if (numFalseSpatial[i]! > 0) meanProbsIfFalseSpatial[i] = sumProbsIfFalseSpatial[i]! / numFalseSpatial[i]!;
    ...
    const trueBranch = ExperienceImbalancedBinaryClassificationTree2Learner.buildNode(
      featureSet,
    ...
    const falseBranch = ExperienceImbalancedBinaryClassificationTree2Learner.buildNode(
      featureSet,
