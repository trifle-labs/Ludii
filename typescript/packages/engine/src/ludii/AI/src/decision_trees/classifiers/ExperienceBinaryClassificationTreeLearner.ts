// @java AI/src/decision_trees/classifiers/ExperienceBinaryClassificationTreeLearner.java

/**
 * Class with methods for learning binary classification trees from experience.
 *
 * @java decision_trees/classifiers/ExperienceBinaryClassificationTreeLearner.java
...
function softmaxInPlace(arr: number[]): void {
  ...
  for (let i = 0; i < arr.length; ++i) {
    (arr as number[])[i] = Math.exp((arr as number[])[i]! - max);
    sum += (arr as number[])[i]!;
  }
 ...
 * Class with methods for learning binary classification trees from experience.
 *
 * @java decision_trees.classifiers.ExperienceBinaryClassificationTreeLearner
 */
export class ExperienceBinaryClassificationTreeLearner {

  //-------------------------------------------------------------------------

  /**
   * Builds a binary classification tree node for given feature set and experience buffer
   * @java ExperienceBinaryClassificationTreeLearner.buildTree(BaseFeatureSet, LinearFunction, ExperienceBuffer, int, int)
   */
  public static buildTree(
    ...
    return ExperienceBinaryClassificationTreeLearner.buildNode(
      featureSet,
      allFeatureVectors,
      allTargetLabels,
      new Set<number>(),
  ...
  /**
   * @java ExperienceBinaryClassificationTreeLearner.buildNode(BaseFeatureSet, List<FeatureVector>, TFloatArrayList, BitSet, BitSet, int, int, int, int)
   */
  private static buildNode(
    ...
    const meanProbsIfFalseAspatial = new Array<number>(numAspatialFeatures).fill(0) as number[];
    const meanProbsIfTrueAspatial = new Array<number>(numAspatialFeatures).fill(0) as number[];
    const meanProbsIfFalseSpatial = new Array<number>(numSpatialFeatures).fill(0) as number[];
    const meanProbsIfTrueSpatial = new Array<number>(numSpatialFeatures).fill(0) as number[];

    for (let i = 0; i < numAspatialFeatures; ++i) {
      if ((numFalseAspatial as number[])[i]! > 0) meanProbsIfFalseAspatial[i] = (sumProbsIfFalseAspatial as number[])[i]! / (numFalseAspatial as number[])[i]!;
      if ((numTrueAspatial as number[])[i]! > 0) meanProbsIfTrueAspatial[i] = (sumProbsIfTrueAspatial as number[])[i]! / (numTrueAspatial as number[])[i]!;
    }

    for (let i = 0; i < numSpatialFeatures; ++i) {
      if ((numFalseSpatial as number[])[i]! > 0) meanProbsIfFalseSpatial[i] = (sumProbsIfFalseSpatial as number[])[i]! / (numFalseSpatial as number[])[i]!;
      if ((numTrueSpatial as number[])[i]! > 0) meanProbsIfTrueSpatial[i] = (sumProbsIfTrueSpatial as number[])[i]! / (numTrueSpatial as number[])[i]!;
    }
    ...
    for (let i = 0; i < numAspatialFeatures; ++i) {
      ...
      for (let j = 0; j < remainingFeatureVectors.length; ++j) {
        ...
        const error = fv.aspatialFeatureValues().get(i) !== 0
          ...
          : targetProb - meanProbsIfFalseAspatial[i]!;
    ...
    for (let i = 0; i < numSpatialFeatures; ++i) {
      ...
      for (let j = 0; j < remainingFeatureVectors.length; ++j) {
        ...
        const error = fv.activeSpatialFeatureIndices().contains(i)
          ...
          : targetProb - meanProbsIfFalseSpatial[i]!;
        sumSquaredErrors += error * error;
      }
    ...
    const trueBranch = ExperienceBinaryClassificationTreeLearner.buildNode(
      featureSet,
    ...
    const falseBranch = ExperienceBinaryClassificationTreeLearner.buildNode(
      featureSet,
