// @java AI/src/training/feature_discovery/CorrelationBasedExpander.java
 ...
 *
 * @java training/feature_discovery/CorrelationBasedExpander.java
...
// Escape-hatch for not-yet-ported Gradients
type GradientsT = {
...
// Simple Map-based replacement for TObjectIntHashMap / TObjectDoubleHashMap
// using the pair's hashCode as key
class ObjIntMap {
  private map: Map<number, Array<{ key: CombinableFeatureInstancePair; value: number }>>;
...
/**
 * A simple Set-like container using hashCode + equals for CombinableFeatureInstancePair.
 */
...
/**
 * A simple priority queue (max-heap by score) for ScoredFeatureInstancePair.
 */
...
/**
 * Correlation-based Feature Set Expander.
 *
 * @java training.feature_discovery.CorrelationBasedExpander
 */
export class CorrelationBasedExpander implements FeatureSetExpander {

  /**
   * @java CorrelationBasedExpander.expandFeatureSet(...)
   */
  public expandFeatureSet(
    ...
    const apprenticePolicies: FVectorT[] = new Array(batch.length);
    ...
    batchIndices.sort((o1, o2) => {
      const deltaAbsErrorSums = absErrorSums[o1]! - absErrorSums[o2]!;
      if (deltaAbsErrorSums > 0.0) return -1;
      else if (deltaAbsErrorSums < 0.0) return 1;
      else return 0;
    });
    ...
    for (let bi = 0; bi < batchIndices.length; ++bi) {
      ...
      for (let aIdx = 0; aIdx < sortedActionIndices.length; ++aIdx) {
        ...
        // list --> set --> list to get rid of duplicates (using FeatureInstance reference equality for now)
        ...
        if (numInstancesAllowedThisAction > 0) {
          ...
          const distrVals: number[] = new Array(activeInstances.length);
          for (let i = 0; i < activeInstances.length; ++i) {
            const fIdx = activeInstances[i]!.feature().spatialFeatureSetIndex();
            distrVals[i] = expectedAbsErrorGivenFeature[fIdx]!;
          }

          // Softmax with temperature 2.0
          const maxVal = Math.max(...distrVals);
          let sumExp = 0.0;
          const expVals: number[] = distrVals.map(v => { const e = Math.exp((v - maxVal) * 2.0); sumExp += e; return e; });
          const distr: number[] = expVals.map(e => e / sumExp);
          ...
          // Mark as invalid (0) once sampled
    ...
    // Keep trying to add a proactive feature, until we succeed
    ...
    // Keep trying to add a reactive feature, until we succeed
