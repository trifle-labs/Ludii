// @java AI/src/training/feature_discovery/SpecialMovesCorrelationExpander.java

/**
 * Feature Set Expander that simply likes correlations with special moves
 * (winning moves, losing moves, anti-defeating moves).
 *
 * @java training/feature_discovery/SpecialMovesCorrelationExpander.java
 ...
 * (winning moves, losing moves, anti-defeating moves).
 *
 * @java training.feature_discovery.SpecialMovesCorrelationExpander
 */
export class SpecialMovesCorrelationExpander implements FeatureSetExpander {

  /**
   * @java SpecialMovesCorrelationExpander.expandFeatureSet(...)
   */
  public expandFeatureSet(
    ...
    // For every sample in batch, first compute errors and sum of absolute errors
    ...
    for (let i = 0; i < batch.length; ++i) {
      ...
      // Construct errors based on special moves
      const errorsArr: number[] = new Array(featureVectors.length);
      for (let a = 0; a < featureVectors.length; ++a) {
        let actionError: number;
      ...
      // Build FVector-like objects from errorsArr
      const errVec = buildFVector(errorsArr);
      const absErrVec = buildFVector(errorsArr.map(Math.abs));

      errorVectors[i] = errVec;
      absErrorSums[i] = absErrVec.sum();
    }

    // For every feature, compute correlation metrics
    const featureErrorCorrelations: number[] = new Array(featureSet.getNumSpatialFeatures()).fill(0.0);
    const expectedAbsErrorGivenFeature: number[] = new Array(featureSet.getNumSpatialFeatures()).fill(0.0);
    const expectedFeatureTimesAbsError: number[] = new Array(featureSet.getNumSpatialFeatures()).fill(0.0);
    ...
    for (let fIdx = 0; fIdx < featureSet.getNumSpatialFeatures(); ++fIdx) {
      ...
      if (isNaN(featureErrorCorrelations[fIdx]!)) {
        featureErrorCorrelations[fIdx] = 0.0;
      }
    }
    ...
    for (let bi = 0; bi < batchIndices.length; ++bi) {
      ...
      for (let aIdx = 0; aIdx < sortedActionIndices.length; ++aIdx) {
        ...
        if (numInstancesAllowedThisAction > 0) {
          // Create distribution proportional to expectedAbsErrorGivenFeature
...
// Helper: create a minimal FVector-like from an array of numbers
function buildFVector(arr: number[]): FVectorT {
  return {
    get(i: number): number { return arr[i]!; },
    set(i: number, v: number): void { arr[i] = v; },
    copy(): FVectorT { return buildFVector([...arr]); },
    abs(): void { for (let i = 0; i < arr.length; ++i) arr[i] = Math.abs(arr[i]!); },
    sum(): number { return arr.reduce((a, b) => a + b, 0); },
  };
}
