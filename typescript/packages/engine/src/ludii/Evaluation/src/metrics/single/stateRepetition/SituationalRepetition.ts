// @java Evaluation/src/metrics/single/stateRepetition/SituationalRepetition.java

/**
 * Percentage number of repeated situational states.
 *
 * @java metrics/single/stateRepetition/SituationalRepetition.java
...
/** @java other/concept/Concept.SituationalRepetition */
const ConceptSituationalRepetition: Concept = "SituationalRepetition" as unknown as Concept;
 ...
 * @java metrics/single/stateRepetition/SituationalRepetition.java
 */
export class SituationalRepetition extends Metric {

  //-------------------------------------------------------------------------

  /**
   * Constructor.
   * @java SituationalRepetition()
   */
  public constructor() {
    super(
  ...
  /**
   * @java SituationalRepetition.apply(Game, Evaluation, Trial[], RandomProviderState[])
   */
  public apply(
    game: Game,
    ...
    for (let trialIndex = 0; trialIndex < trials.length; trialIndex++) {
      ...
      trialStates.push(ctx.state().fullHash());
      trialStateCounts.push(1);

      for (const m of trial.generateRealMovesList()) {
        g.apply(context, m);

        const currentState: bigint = ctx.state().fullHash();
  ...
  /**
   * @java SituationalRepetition.startNewTrial(Context, Trial)
   */
  public startNewTrial(_context: Context, _fullTrial: Trial): void {
    console.error("Incrementally computing metric not yet implemented for SituationalRepetition.");
  }

  /**
   * @java SituationalRepetition.observeNextState(Context)
   */
  ...
  /**
   * @java SituationalRepetition.observeFinalState(Context)
   */
  ...
  /**
   * @java SituationalRepetition.finaliseMetric(Game, int)
   */
