// @java Common/src/graphics/qr_codes/QrSegmentAdvanced.java
 ...
 *
 * @java graphics/qr_codes/QrSegmentAdvanced.java
 */
export class QrSegmentAdvanced {
  /*---- Optimal list of segments encoder ----*/

  /**
   * Returns a list of zero or more segments to represent the specified Unicode text string,
   * optimally minimizing the total encoded bit length.
   * @java QrSegmentAdvanced.makeSegmentsOptimally(String, QrCode.Ecc, int, int)
   */
  public static makeSegmentsOptimally(text: string, ecl: Ecc, minVersion: number, maxVersion: number): QrSegment[] {
    ...
    let segs: QrSegment[] | null = null;
    const codePoints = QrSegmentAdvanced.toCodePoints(text);
    for (let version = minVersion; ; version++) {
      if (version === minVersion || version === 10 || version === 27) {
        segs = QrSegmentAdvanced.makeSegmentsOptimallyInternal(codePoints, version);
      }
      if (!segs) throw new Error("segs should not be null");
   ...
   * @java QrSegmentAdvanced.makeSegmentsOptimally(int[], int)
   */
  private static makeSegmentsOptimallyInternal(codePoints: number[], version: number): QrSegment[] {
    if (codePoints.length === 0) return [];
    const charModes = QrSegmentAdvanced.computeCharacterModes(codePoints, version);
    return QrSegmentAdvanced.splitIntoSegments(codePoints, charModes);
  }

  /**
   * Returns a new array representing the optimal mode per code point.
   * @java QrSegmentAdvanced.computeCharacterModes(int[], int)
   */
  private static computeCharacterModes(codePoints: number[], version: number): Mode[] {
    if (codePoints.length === 0) throw new Error("codePoints is empty");
    const modeTypes = [Mode.BYTE, Mode.ALPHANUMERIC, Mode.NUMERIC, Mode.KANJI];
    const numModes = modeTypes.length;

    // Segment header sizes, measured in 1/6 bits
    const headCosts: number[] = [];
    for (let i = 0; i < numModes; i++) {
      const m = modeTypes[i];
      if (!m) throw new Error("modeTypes access out of range");
      headCosts[i] = (4 + m.numCharCountBits(version)) * 6;
    }

    // charModes[i][j]: mode to encode code point at index i such that final segment ends in modeTypes[j]
    const charModes: (Mode | null)[][] = [];
    for (let i = 0; i < codePoints.length; i++) {
      charModes.push(new Array(numModes).fill(null));
    }

    let prevCosts: number[] = [...headCosts];

    for (let i = 0; i < codePoints.length; i++) {
      const c = codePoints[i];
      if (c === undefined) throw new Error("codePoints access out of range");
      const curCosts: number[] = new Array(numModes).fill(0);
      const row = charModes[i];
      if (!row) throw new Error("charModes access out of range");

      // Always extend a byte mode segment
      curCosts[0] = (prevCosts[0] ?? 0) + QrSegmentAdvanced.countUtf8Bytes(c) * 8 * 6;
      row[0] = modeTypes[0] ?? null;

      // Extend a segment if possible
      const alphaMap = QrSegment.ALPHANUMERIC_MAP;
      if (c < alphaMap.length) {
        const av = alphaMap[c];
        if (av !== undefined && av !== -1) {
          curCosts[1] = (prevCosts[1] ?? 0) + 33;
          row[1] = modeTypes[1] ?? null;
        }
      }
      if (c >= 0x30 && c <= 0x39) {  // '0'-'9'
        curCosts[2] = (prevCosts[2] ?? 0) + 20;
        row[2] = modeTypes[2] ?? null;
      }
      if (QrSegmentAdvanced.isKanji(c)) {
        curCosts[3] = (prevCosts[3] ?? 0) + 78;
        row[3] = modeTypes[3] ?? null;
      }
      ...
      for (let j = 0; j < numModes; j++) {
        for (let k = 0; k < numModes; k++) {
          ...
          const hj = headCosts[j];
          if (hj === undefined) continue;
          const newCost = Math.floor((ck + 5) / 6) * 6 + hj;
          const rk = row[k];
          const rj = row[j];
          if (rk !== null && (rj === null || newCost < (curCosts[j] ?? 0))) {
            curCosts[j] = newCost;
            row[j] = modeTypes[k] ?? null;
          }
        }
      }
    ...
    let minCost = 0;
    for (let i = 0; i < numModes; i++) {
      const pc = prevCosts[i];
      if (pc === undefined) continue;
      if (curMode === null || pc < minCost) {
        minCost = pc;
        curMode = modeTypes[i] ?? null;
      }
    }
   ...
   * @java QrSegmentAdvanced.splitIntoSegments(int[], Mode[])
   */
  private static splitIntoSegments(codePoints: number[], charModes: Mode[]): QrSegment[] {
    if (codePoints.length === 0) throw new Error("codePoints is empty");
    const result: QrSegment[] = [];

    let curMode = charModes[0];
    if (!curMode) throw new Error("charModes[0] is null");
    let start = 0;
    for (let i = 1; ; i++) {
      if (i < codePoints.length && charModes[i] === curMode) continue;
      const sub = codePoints.slice(start, i);
      const s = String.fromCodePoint(...sub);
      if (curMode === Mode.BYTE) {
        result.push(QrSegment.makeBytes(new TextEncoder().encode(s)));
      } else if (curMode === Mode.NUMERIC) {
        result.push(QrSegment.makeNumeric(s));
      } else if (curMode === Mode.ALPHANUMERIC) {
        result.push(QrSegment.makeAlphanumeric(s));
      } else if (curMode === Mode.KANJI) {
        result.push(QrSegmentAdvanced.makeKanji(s));
      } else {
        throw new Error("Unexpected mode");
      }
      if (i >= codePoints.length) return result;
      curMode = charModes[i] as Mode;
      start = i;
    }
  }

  /**
   * Returns a new array of Unicode code points representing the given UTF-16 string.
   * @java QrSegmentAdvanced.toCodePoints(String)
   */
  private static toCodePoints(s: string): number[] {
    ...
    for (const char of s) {
      ...
      if (cp >= 0xD800 && cp <= 0xDFFF) throw new Error("Invalid UTF-16 string");
      result.push(cp);
    }
    return result;
  }
   ...
   * @java QrSegmentAdvanced.countUtf8Bytes(int)
   */
  private static countUtf8Bytes(cp: number): number {
    if      (cp <       0) throw new Error("Invalid code point");
   ...
   * @java QrSegmentAdvanced.makeKanji(String)
   */
  public static makeKanji(text: string): QrSegment {
    if (text === null || text === undefined) throw new Error("text is null");
    const bb = new BitBuffer();
    for (const char of text) {
      const c = char.codePointAt(0);
      if (c === undefined) throw new Error("Invalid string");
      if (c >= QrSegmentAdvanced.UNICODE_TO_QR_KANJI.length) {
        throw new Error("String contains non-kanji-mode characters");
      }
      const val = QrSegmentAdvanced.UNICODE_TO_QR_KANJI[c];
   ...
   * @java QrSegmentAdvanced.isEncodableAsKanji(String)
   */
  public static isEncodableAsKanji(text: string): boolean {
    if (text === null || text === undefined) throw new Error("text is null");
    for (const char of text) {
      const c = char.codePointAt(0);
      if (c === undefined) return false;
      if (!QrSegmentAdvanced.isKanji(c)) return false;
    }
    return true;
  }

  /** @java QrSegmentAdvanced.isKanji(int) */
  private static isKanji(c: number): boolean {
    return c < QrSegmentAdvanced.UNICODE_TO_QR_KANJI.length
      && QrSegmentAdvanced.UNICODE_TO_QR_KANJI[c] !== -1;
  }
   ...
   * @java QrSegmentAdvanced.UNICODE_TO_QR_KANJI
   */
  private static readonly UNICODE_TO_QR_KANJI: readonly number[] = (() => {
    // The packed base64 string from the Java source (verbatim)
    const PACKED_QR_KANJI_TO_UNICODE =
      ...
      "JRglFCUcJSwlJCU0JTwlASUDJQ8lEyUbJRclIyUzJSslOyVLJSAlLyUoJTclPyUdJTBlJSU4JUL//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      ...
      "csNyxnLEcs5y0nLicuBy4XL5cvdQD3MXcwpzHHMWcx1zNHMvcylzJXM+c05zT57Yc1dzanNoc3BzeHN1c3tzenPIc7NzznO7c8Bz5XPuc940onQFdG90JXP4dDJ0OnRVdD90X3RZdEF0XHRpdHB0Y3RqdHZ0fnSLdJ50p3TKdM901HPx////////dOB043TndOl07nTy" +
      ...
      "djt2R3ZIdkZ2XHZYdmF2YnZodml2anZndmx2cHZydnZ2eHZ8doB2g3aIdot2jnaWdpN2mXaadrB2tHa4drl2unbCds121nbSdt524Xbldud26oYvdvt3CHcHdwR3KXckdx53JXcmdxt3N3c4d0d3Wndod2t3W3dld393fnd5d413i3eRd6B3nnewd7Z3uXe/d7x3vXe7" +
      "d8d3zXfXd9p33Hfjd+53/HgMeBJ5JnggeSp4RXiOeHR4hnh8eJp4jHijeLV4qniveNF4xnjLeNR4vni8eMV4ynjs////////eOd42nj9ePR5B3kSeRF5GXkseSt5QHlgeVd5X3laeVV5U3l6eX95inmdeaefS3mqea95s3m5ebp5yXnVeed57HnheeN6CHoNehh6GXog" +
      ...
      "e5p7i3uSe497XXuZe8t7wXvMe897tHvGe9176XwRfBR75nvlfGB8AHwHfBN883v3fBd8DXv2fCN8J3wqfB98N3wrfD18THxDfFR8T3xAfFB8WHxffGR8VnxlfGx8dXyDfJB8pHytfKJ8q3yhfKh8s3yyfLF8rny5fL18wHzFfMJ82HzSfNx84ps7fO988nz0fPZ8+n0G" +
      ...
      "mdGZ7ZnumfGZ8pn7mfiaAZoPmgWZ4poZmiuaN5pFmkKaQJpD//+aPppVmk2aW5pXml+aYpplmmSaaZprmmqarZqwmryawJrPmtGa05rUmt6a35rimuOa5prvmuua7pr0mvGa95r7mwabGJsamx+bIpsjmyWbJ5tomymbKpsumy+bMptEm0ObT5tNm06bUZtYm3Sbk5uD" +
      ...
      "n0qfUp9Un2OfX59gn2GfZp9nn2yfaq93n3Kfdp+Vn5yfoFgvaceQWXRkUdxxmf//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
      "/////////////////////////////////////////////w==";
  ...
  /** Not instantiable. @java QrSegmentAdvanced() */
  private constructor() {}
}
